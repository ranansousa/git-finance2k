  ...
  dmConexao.conectarBanco;
    ...
    if not uutil.Empty(titulo.FID) then begin
        ...
        if TTituloPagarDAO.existFilhos(titulo) then begin
          sucesso := False;
          msgGenerica := 'Existem outros títulos vinculados. Não será possível excluir.';
          ...
          if qry.RecordCount >0  then begin
             ...
             loMostraErros.Add(' Existem ' + IntToStr(qry.RecordCount) + ' títulos vinculados ' );
        ...
        begin
         ...
         sucesso :=   PempecMsg.MsgConfirmation('ATENÇÃO. Tem certeza que desejar deletar o registo? Não será possível reverter a operação. Faça Backup antes. ')= 6;
    ...
    if not uutil.Empty(titulo.FID) then begin
      ...
      try
       ...
       {  qry.ParamByName('PID').AsString := titulo.FID;
        ...
        if PempecMsg.MsgConfirmation('O título e tudo que for relacionado a ele será removido totalmente da base de dados.' +#13 + ' Confirma a operação de exclusão total ?' ) = 6  then begin

            qry.ExecSQL;
            dmConexao.conn.Commit;
            PempecMsg.MsgInformation('Titulo excluído da base de dados.  ' + titulo.FnumeroDocumento);
