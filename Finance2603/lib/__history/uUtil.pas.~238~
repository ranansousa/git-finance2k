...
uCriptografia, uMensagens, System.Classes, Printers, wininet, Winsock, IdSSL, IdSSLOpenSSL, IdMessage, IniFiles,
...
type
  ...
  public
    ...
    class function getExtratoBancario: string;
...
begin
  ...
  sKinDefault := arquivoConfig.ReadString('SKIN','SkinName','Skins não encontrados' );
...
begin
  ...
  begin
    try
      ...
      raise Exception.Create('Erro componente : ' + TCustomEdit(Form.Components[i]).Name);
    ...
    try
      ...
      raise Exception.Create('Erro componente : ' + TEdit(Form.Components[i]).Name);
    end;
    ...
    try
      ...
      raise Exception.Create('Erro componente : ' + TComboBox(Form.Components[i]).Name);
    ...
    try
      ...
      raise Exception.Create('Erro componente : ' + TDateTimePicker(Form.Components[i]).Name);
...
class function TPathRelatorio.getExtratoBancario: string;
begin
Result := getPathRelatorios + 'extratoBancario.fr3';
end;
