...
centroCusto : TTituloPagarCentroCustoModel;
 qtdRateioC, qtdRateioH, I :Integer;
valorSalvo, valorExtra, valorAC, valorDE, valorNominal, valorAntecipado :Currency;
...
// dmConexao.conn.StartTransaction;
    ...
    if Insert(baixa) then begin
      ...
      begin
        ...
        if valorExtra > 0 then begin

           for I := 0 to titulo.listaCustos.Count - 1 do
          begin
              titulo.listaCustos[I].Fvalor := ( baixa.FvalorPago / titulo.listaCustos.Count);
              valorSalvo := valorSalvo + titulo.listaCustos[I].Fvalor;
              sucesso := TTituloPagarCentroCustoDAO.Update(titulo.listaCustos[I]);
          end;


          if valorSalvo <> baixa.FvalorPago then begin

            valorExtra := valorSalvo - baixa.FvalorPago;
            titulo.listaCustos[0].Fvalor := titulo.listaCustos[0].Fvalor + valorExtra;
