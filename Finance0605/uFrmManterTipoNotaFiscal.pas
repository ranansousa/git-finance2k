...
type
  TfrmManterTipoNotaFiscal = class(TForm)
...
var
  frmManterTipoNotaFiscal: TfrmManterTipoNotaFiscal;

implementation

{$R *.dfm}

procedure TfrmManterTipoNotaFiscal.cxbrdtmPesquisaCurChange(Sender: TObject);
begin
  dbgrd1.DataSource.DataSet.Locate('DESCRICAO',UpperCase(cxbrdtmPesquisa.CurEditValue),[loPartialKey]);
end;

procedure TfrmManterTipoNotaFiscal.dbgrd1TitleClick(Column: TColumn);
begin
 (dbgrd1.DataSource.DataSet as TFDQuery).IndexFieldNames :=Column.FieldName;
end;

function TfrmManterTipoNotaFiscal.deletar(pIdOrganizacao, pIdValue: string): Boolean;
var cmd :string;
qryDel :TFDQuery;
begin
...
procedure TfrmManterTipoNotaFiscal.dsGridDataChange(Sender: TObject; Field: TField);
begin
...
procedure TfrmManterTipoNotaFiscal.dxBtnDeletarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoNotaFiscal.dxBtnEditarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoNotaFiscal.dxBtnFecharClick(Sender: TObject);
begin
 PostMessage(Self.Handle, WM_CLOSE, 0, 0);
end;

procedure TfrmManterTipoNotaFiscal.dxBtnNovoClick(Sender: TObject);
begin
...
procedure TfrmManterTipoNotaFiscal.dxBtnSalvarClick(Sender: TObject);
var
 pDescricao :string;
begin
...
procedure TfrmManterTipoNotaFiscal.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 freeAndNilDM(Self);
 Action :=caFree;
...
procedure TfrmManterTipoNotaFiscal.FormCreate(Sender: TObject);
begin
 pTable := 'TIPO_NOTA_FISCAL';
 Self.Caption := pTable;
 dxRibbon1Tab1.Caption := 'Manutenção de tipos de nota fiscal';

 inicializarDM(Self);
 limparPanel;
end;

procedure TfrmManterTipoNotaFiscal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 case key of
...
procedure TfrmManterTipoNotaFiscal.freeAndNilDM(Sender: TObject);
begin
...
procedure TfrmManterTipoNotaFiscal.inicializarDM(Sender: TObject);
begin
 //nada
...
procedure TfrmManterTipoNotaFiscal.limparPanel();
var
  I: Integer;
begin
 limpaStatusBar;
...
procedure TfrmManterTipoNotaFiscal.msgStatusBar(pPosicao : Integer; msg :string);
begin
dxStatusBar.Panels[pPosicao].Text := msg;
Application.ProcessMessages;
end;

procedure TfrmManterTipoNotaFiscal.preencheGrid(pIdOrganizacao: string);
var
sqlCmd :string;
begin
...
function TfrmManterTipoNotaFiscal.liberaDeletar(cmd :string) : Integer;
var
...
function TfrmManterTipoNotaFiscal.qtdUso(pIdOrganizacao,  pIdValue: string): Integer;
  var
qryPesquisa, qryTR : TFDQuery;
cmd : string;
qtd :Integer;
begin
  ...
  try
      ...
      cmd := ' SELECT FIRST 1  CBD.ID_' + pTable + ' FROM NOTA_FISCAL_ENTRADA CBD WHERE (CBD.ID_ORGANIZACAO =:PIDORGANIZACAO) AND (CBD.ID_'+pTable + '  = :PID) ' ;
...
procedure TfrmManterTipoNotaFiscal.limpaStatusBar;
begin
dxStatusBar.Panels[0].Text := 'Status ';
dxStatusBar.Panels[1].Text := 'Ativo. Teclas de atalho:  [F2] = Editar  - [F4] = Novo - [F10] = Salvar  ';
end;

function  TfrmManterTipoNotaFiscal.salvarContaBancaria(PIDORGANIZACAO, PIDVALUE, PDESCRICAO  :string) :Boolean;
var
cmdSalvar :string;
qrySalvar :TFDQuery;
begin
...
procedure TfrmManterTipoNotaFiscal.LimpaTela(Form: TForm);
var
  i: Integer;
begin
...
procedure TfrmManterTipoNotaFiscal.controleEdit(Form: TForm; pValue :Boolean);
var
i: Integer;
begin
