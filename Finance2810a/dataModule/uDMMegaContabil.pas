...
begin
  ...
  try
     ...
     qryGravaLote.SQL.Clear;
      ...
      if pDebito >0 then begin

       cmdUpdate := ' UPDATE CLOTES SET DEBITO = :PDEB, TOTAL = :PTOTAL  WHERE ID = :PID ';
       qryGravaLote.ParamByName('PDEB').AsCurrency := pDebito;
       qryGravaLote.ParamByName('PTOTAL').AsCurrency := pDebito;

      end;

       if pCredito >0 then begin

       cmdUpdate := ' UPDATE CLOTES SET CREDITO = :PCRE, TOTAL = :PTOTAL WHERE ID = :PID ';
