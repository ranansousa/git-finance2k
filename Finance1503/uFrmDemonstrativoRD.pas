...
uses
  ...
  System.Variants, System.Classes, Vcl.Graphics,uUtil,
...
begin
 ...
 // PempecMsg.MsgInformation('Rotina em desenvolvimento. Por favor, aguarde!');
  ...
  frmConsolidadoCC := TfrmConsolidadoCC.Create(Self);
...
edtValorReceita.Text :=  FormatFloat('#,.##',vlrReceitas);
 ...
 msgStatusBar(3, 'Processando a solicitação...');
   ...
   if validarPeriodo then begin
      ...
      if (dmRD.obterDespesas(pIdOrganizacao,dtpDataInicial.Date, dtpDataFinal.Date)) then begin
      ...
      if (dmRD.obterReceitas(pIdOrganizacao,dtpDataInicial.Date, dtpDataFinal.Date)) then begin
       ...
       registraMovimentacao(pIdOrganizacao, 'CENTRO_CUSTO', 'DEMONSTRATIVO RD', uutil.TUserAtual.getNameUser + ' solicitou DEMONSTRATIVO RD'  , 'SOLICITADO');
