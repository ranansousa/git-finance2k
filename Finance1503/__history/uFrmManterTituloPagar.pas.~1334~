...
uses
  ...
  System.Generics.Collections,  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, udmConexao, uOrganizacaoModel, uDMOrganizacao,uUtil,
  ...
  cxLookAndFeels, cxLookAndFeelPainters, dxRibbonSkins, uTipoStatusModel, uTipoStatusDAO,  dxRibbonCustomizationForm,
  dxBarBuiltInMenu, Data.DB, FireDAC.Stan.Intf,
  FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt,
  uFrmManterHistorico, uFrmManterCedente,
  FireDAC.Comp.Client, FireDAC.Comp.DataSet, dxBar, cxClasses, Vcl.StdCtrls,
  Vcl.Grids, Vcl.DBGrids, uFrameTipoCobranca,UMostraErros,
 ...
 uTPBaixaDAO, uTPBaixaChequeDAO, uTPBaixaFPDAO, uTPBaixaACDAO, uTPBaixaDEDAO, uTPBaixaInternetDAO,
  EMsgDlg, uMD5, Vcl.ExtCtrls, dxSkinBlack, dxSkinBlue, dxSkinBlueprint,
  ...
  dxSkinVisualStudio2013Blue, dxSkinOffice2019Colorful,
...
type
  TfrmManterTituloPagar = class(TForm)
    ...
    PempecMsg: TEvMsgDlg;
    edtDataAtualiza: TEdit;
    Label43: TLabel;
    painel: TMemo;
    edtValorPago: TEvNumEdit;
    Label42: TLabel;
    chkProvisionavel: TCheckBox;
    qryVerificaBaixa: TFDQuery;
    qryBaixaPorContrato: TFDQuery;
    cxbaixas: TcxTabSheet;
    dbgBaixas: TDBGrid;
    ...
    procedure tbRateioCentroCustoExit(Sender: TObject);
    procedure tbRateioHistoricosExit(Sender: TObject);
  ...
  private
   ...
   msg,  pIDusuario :string;
    ...
    function obterIndiceListaPorDOC(pDOC: string): Integer;
    function verificaBaixa(pIdOrganizacao, pid: string): Boolean;
    function totalBaixaPorContrato(pIdOrganizacao, pIdCedente, pContrato,
      pParcela: string): Currency; //retorna o index do combo onde tem o documento
...
begin
  ...
  begin
   ...
   if Assigned(tituloPagarModel.listaCustos) then begin
     tituloPagarModel.listaCustos.Clear;
   end;
...
begin
  ...
  begin //atualiza saldo do rateio CC
    ...
    begin
      edtDifRateioCC.Value := edtValorPago.Value - totalRateio;
    end;
...
begin
  ...
  begin
    PempecMsg.MsgInformation('O centro de custo não pode ser utilizado. Possível problema com a consulta . ');
    frameCentroCusto2.cbbcombo.ItemIndex := 0;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      PempecMsg.MsgInformation('O histórico não pode ser utilizado. Possível problema no campo conta contábil. ');
      frameHistorico2.cbbcombo.ItemIndex := 0;
...
begin
   ...
   ajustaComboForm;
    ...
    if uutil.Empty(titulo.FID) then begin
          ...
          if not frameTP1.obterTodosInconsistentes(pIDorganizacao, frameTP1.cbbTP, FSlistaTitulos) then
          begin
...
procedure TfrmManterTituloPagar.tbBasicaShow(Sender: TObject);
begin


  if Assigned(tituloPagarModel) then
  begin

    if tituloPagarModel.Fnovo then
    begin
      tituloPagarModel.FTipoStatus := TTipoStatusModel.Create;
      tituloPagarModel.FTipoStatus.FIDorganizacao := pIDorganizacao;
      tituloPagarModel.FTipoStatus.FID := 'ABERTO';
    end;
    popularStatus(tituloPagarModel.FTipoStatus);

  end;

  msgStatusBar(3, ' F5 - Históricos - F6 - Cedentes  ' + uutil.TOrgAtual.getOrganizacao.FSigla);

// msgStatusBar(3,'Aba básica.  CTRL + F12 carrega todos os títulos da ' + uutil.TOrgAtual.getOrganizacao.FSigla);
...
procedure TfrmManterTituloPagar.tbComplementoShow(Sender: TObject);
begin
 popularStatus(tituloPagarModel.FTipoStatus);
 msgStatusBar(3,'Modo de edição. Para Salvar retorne a aba básica.');
end;
...
begin
  ...
  if not uUtil.Empty(tituloPagarModel.FIDNotaFiscalEntrada) then begin
    ...
    notaFiscal.FIDusuario         := uutil.TUserAtual.getUserId;
...
procedure TfrmManterTituloPagar.tbRateioCentroCustoExit(Sender: TObject);
begin
StatusBotoes(1);
end;
...
procedure TfrmManterTituloPagar.tbRateioHistoricosExit(Sender: TObject);
begin
StatusBotoes(1);
end;
...
var
  ...
  controlePagto :Integer; //apenas soma quantos pagamentos foram cancelados.
...
painel.Clear;
  ...
  begin
    ...
    permite := False;
    msg := 'O título foi pago em lote. Primeiro deve remover o lote pagamento : ' + edtLotePagamento.Text;
    PempecMsg.MsgInformation(msg);

    painel.Lines.Add(msg);
 ...
 { ver //ranan em 30/06
  ...
  begin
    ...
    permite := False;
    msg := 'O título já foi exportado. Primeiro deve remover o lote contábil. ' + edtLoteContabil.Text;
    PempecMsg.MsgInformation(msg);
    painel.Lines.Add(msg);
  ...
  begin
    ...
    begin
      ...
      begin
    //    if MessageDlg('O título será automaticamente provisionado. Deseja cancelar o pagamento? ', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
           msg := 'O título será automaticamente provisionado. Deseja cancelar o pagamento? ';
 ...
 if permite then
  begin
    dmConexao.conectarBanco;
      ...
      painel.Lines.Add('Pesquisando dados da baixa do titulo '+ tituloPagarModel.FnumeroDocumento);
      ...
      if not uutil.Empty(tpBaixaModel.FID) then begin
       ...
       if TTPBaixaDEDAO.deletePorTPBaixa(deducao) then begin painel.Lines.Add('Deduções canceladas'); end;
      ...
      if TTPBaixaACDAO.deletePorTPBaixa(acrescimo) then begin painel.Lines.Add('Acréscimos cancelados'); end;
       ...
       if not tpbCheques.IsEmpty then begin
         ...
         while not tpbCheques.Eof do begin
            ...
            painel.Lines.Add('Pesquisando dados em baixa de cheques ');
            ...
            if not uutil.Empty(pagtoCheque.FID) then begin
                ...
                if cheque.FIDtipoStatus.Equals('COMPENSADO') then begin compensado := True; end;
                ...
                if not uutil.Empty(cheque.FID) then begin
                    ...
                    cheque.FIDTOB := '';
                    cheque.Fobservacao := '';
                    cheque.FIDresponsavel := FSlistaResponsavel[frameResponsavel1.cbbcombo.ItemIndex];
                    cheque.Fportador := '';
                   //ver isso depois. no cheque está fazendo commit


                   if compensado then begin
                    if TContaBancariaChequeDAO.estornarCompensado(cheque) then begin painel.Lines.Add('Cheque estornado'); end;

                   end else begin TContaBancariaChequeDAO.Update(cheque); end;

                end;

             if TTPBaixaChequeDAO.Delete(pagtoCheque) then begin Inc(controlePagto); end;
        ...
        tpbWWW := TTPBaixaInternetDAO.obterTodosPorBaixa(pagtoWWW);
         ...
         while not tpbWWW.Eof do begin
            ...
            if not uUtil.Empty(pagtoWWW.FID) then begin
               ...
               if TTPBaixaInternetDAO.Delete(pagtoWWW) then begin
                    painel.Lines.Add('Baixa via internet cancelada');
               Inc(controlePagto); end;
       ...
       if TTesourariaDBDAO.deleteTodosPorTPB(debitoCaixa) then begin painel.Lines.Add('Débitos ao caixa foram cancelados'); end;
        ...
        begin
         //alterar o tp
          ...
          tituloPagarModel.FdataUltimaAtualizacao := Now;
          //aqui
          tituloPagarModel.FIDUsuario := IntToStr(uutil.TUserAtual.getUserId);


          if TTituloPagarDAO.Update(tituloPagarModel) then
          begin
            painel.Lines.Add('Titulo alterado com sucesso. '); end;

           //deletar o tpb
            try
              sucesso := TTPBaixaDAO.DeletePorTP(tpBaixaModel);

             // sucesso := TTPBaixaDAO.Delete(tpBaixaModel); //alterado em

              if sucesso  then begin painel.Lines.Add('A baixa foi completamente cancelada.'); end;

            except
            sucesso :=False;

             sucesso := TTPBaixaDAO.DeletePorTP(tpBaixaModel);
             if sucesso  then begin painel.Lines.Add('As baixas foram completamente canceladas.'); end;
  ...
  begin
    ...
    msg := 'A baixa do título foi cancelada com sucesso. ' + edtLoteContabil.Text;
    PempecMsg.MsgInformation(msg);
    limparPanel;
  ...
  begin
    msg := ' Não foi possível CANCELAR a baixa do título. Entre em contato com o suporte.';
    PempecMsg.MsgInformation(msg);
  end;
 ...
 if frameTP1.cbbTP.ItemIndex >0  then begin
   ...
   if not uutil.Empty(tituloPagarModel.FID) then begin
     try
      ...
      tituloPagarModel.FdataUltimaAtualizacao := Now;
      tituloPagarModel.FIDTituloPagarAnterior :='';
...
limpaRateioHistorico;
limpaRateioCC;
 dxRibbon1Tab1.Caption := ' Manutenção de Títulos a pagar  ';
 Self.Caption := ' MANUTENÇÃO DE PAGAMENTOS  ';
end;
...
function TfrmManterTituloPagar.verificaBaixa(pIdOrganizacao, pid: string) :Boolean;
var
sqlCmd :string;
 sucesso : Boolean;
 valorPago, valorDevido : Currency;

begin
 //retorna true quando encontrar dados da baixa do titulo

 dmConexao.conectarBanco;
   valorPago :=0; valorDevido :=0;
   sucesso := False;
 ...
 try
    ...
    valorPago := qryVerificaBaixa.FieldByName('VALOR_BAIXA').AsCurrency;
    valorDevido := qryVerificaBaixa.FieldByName('VALOR_TITULO').AsCurrency;
...
function TfrmManterTituloPagar.totalBaixaPorContrato(pIdOrganizacao, pIdCedente,pContrato,  pParcela : string) :Currency;
var
sqlCmd :string;
 sucesso : Boolean;
 valorPago : Currency;
...
begin
 ...
 try
    ...
    qryBaixaPorContrato.Close;
    qryBaixaPorContrato.Connection := dmConexao.conn;
    qryBaixaPorContrato.ParamByName('PIDORGANIZACAO').AsString  := pIdOrganizacao;
    qryBaixaPorContrato.ParamByName('PIDCENTE').AsString        := pIdCedente;
    qryBaixaPorContrato.ParamByName('PCONTRATO').AsString       := pContrato;
    qryBaixaPorContrato.ParamByName('PPARCELA').AsString        := pParcela;


    qryBaixaPorContrato.Open;
    valorPago := qryBaixaPorContrato.FieldByName('TOTAL').AsCurrency;
...
procedure TfrmManterTituloPagar.dxBtnPagarClick(Sender: TObject);
 var
  sucesso : Boolean;  aux : Integer;
  existBaixa : Boolean;
  loMostraErros: TFMostraErros;
  tituloExist : TTituloPagarModel;

begin
  sucesso := True;

  loMostraErros := TFMostraErros.Create(Self);   loMostraErros.Clear;
  aux :=0;
  msgStatusBar(3,'Validando dados...');

  tituloExist := TTituloPagarModel.Create;


  if validarForm(Self) then
  begin
     msgStatusBar(3,'Dados validados...');
    // edtStatus.Text := 'QUITADO';
    ...
    begin
     try
        ...
        existBaixa := verificaBaixa(pIdOrganizacao, tituloPagarModel.FID);

        if not existBaixa then
        begin
          if tituloPagarModel.Fnovo then
          begin

            tituloExist :=   TTituloPagarDAO.obterPorNumeroDocumento(tituloPagarModel);

            if not uutil.Empty(tituloExist.FID) then begin
              Inc(aux);
            end;
            ...
            begin
              aux := 0;
              tituloPagarModel.FnumeroDocumento := dmConexao.obterIdentificador('', pIdOrganizacao, 'TP');
              tituloExist := TTituloPagarDAO.obterPorNumeroDocumento(tituloPagarModel);

              if not uutil.Empty(tituloExist.FID) then
              begin
                Inc(aux);
              end;
            ...
            FreeAndNil(tituloExist);

            sucesso := TTituloPagarDAO.Insert(tituloPagarModel);
          end
          else
          begin
            sucesso := TTituloPagarDAO.Update(tituloPagarModel);
          end;
        end
        else
        begin
         //se o tp tiver registro de baixa e o status for aberto. ELe quita.

          if tituloPagarModel.FIDTipoStatus.Equals('ABERTO') then
          begin
            edtStatus.Text := 'QUITADO';
            atualizarTitulo;
            sucesso := TTituloPagarDAO.Update(tituloPagarModel);
        ...
        begin
              ...
              begin
                 //colocar tela de erro para mostrar mais dados


                 loMostraErros.Clear;
                 loMostraErros.Add('Documento possui baixa no sistema. Verifique os dados.');
                 loMostraErros.Add(' '); loMostraErros.Add(' ');

                 loMostraErros.Add('ID            : ' + tituloPagarModel.FID );
                 loMostraErros.Add('Documento     : ' + tituloPagarModel.FnumeroDocumento);
                 loMostraErros.Add('Valor Nominal : ' + CurrToStr(tituloPagarModel.FvalorNominal));
                 loMostraErros.Add('Descrição     : ' + tituloPagarModel.Fdescricao);
                 loMostraErros.Add('Pagamento     : ' + DateToStr(tituloPagarModel.FdataPagamento));

                 loMostraErros.Add(' '); loMostraErros.Add(' ');


                 loMostraErros.Add('--- DADOS DA BAIXA ---');
                 loMostraErros.Add('VALOR DA BAIXA      : ' +CurrToStr(qryVerificaBaixa.FieldByName('VALOR_BAIXA').AsCurrency) );

                                   loMostraErros.Add(' '); loMostraErros.Add(' ');

                 loMostraErros.Add('--- ESPECIE ---');
                 loMostraErros.Add('PAGTO EM ESPECIE    : ' +CurrToStr(qryVerificaBaixa.FieldByName('VALOR_CAIXA').AsCurrency) );
                 loMostraErros.Add('NUM DOC CAIXA       : ' +(qryVerificaBaixa.FieldByName('DOCCAIXA').AsString) );

                                   loMostraErros.Add(' '); loMostraErros.Add(' ');

                 loMostraErros.Add('--- CHEQUE ---');

                 loMostraErros.Add('PAGTO EM CHEQUE     : ' +CurrToStr(qryVerificaBaixa.FieldByName('VALOR_CHEQUE').AsCurrency) );
                 loMostraErros.Add('CONTA BANCARIA      : ' +(qryVerificaBaixa.FieldByName('CHEQUE_CONTA').AsString) );
                 loMostraErros.Add('TITULAR             : ' +(qryVerificaBaixa.FieldByName('CHEQUE_TITULAR').AsString) );
                 loMostraErros.Add('PORTADOR            : ' +(qryVerificaBaixa.FieldByName('PORTADOR').AsString) );
                 loMostraErros.Add('NUMERO CHEQUE       : ' +(qryVerificaBaixa.FieldByName('NUMERO_CHEQUE').AsString) );

                                         loMostraErros.Add(' '); loMostraErros.Add(' ');

                 loMostraErros.Add('--- LANÇAMENTO NA CONTA BANCARIA ---');

                 loMostraErros.Add('VALOR DEBITADO      : ' +CurrToStr(qryVerificaBaixa.FieldByName('VALOR_BANCO').AsCurrency) );
                 loMostraErros.Add('CONTA BANCARIA      : ' +(qryVerificaBaixa.FieldByName('CONTA').AsString) );
                 loMostraErros.Add('OPERAÇÃO BANCÁRIA   : ' +(qryVerificaBaixa.FieldByName('TOPER').AsString) );
                 loMostraErros.Add('TITULAR            : ' +(qryVerificaBaixa.FieldByName('TITULAR').AsString) );
                 loMostraErros.Add('NUMERO CHEQUE       : ' +(qryVerificaBaixa.FieldByName('NUMERO_CHEQUE').AsString) );
                ...
                try
                  ...
                  loMostraErros.caption := 'Aviso - Inconsistências ';
...
begin
   //  pega os dados do form e seta no titulo
    ...
    tituloPagarModel.FdataVencimento        := uUtil.TFormat.proximoDiaUtil(dtPagamento.Date);
    ...
    tituloPagarModel.FprevisaoCartorio      := uUtil.TFormat.proximoDiaUtil(dtPrevisaoCartorio.Date);
    tituloPagarModel.Fdescricao             := edtDescricao.Text;
    tituloPagarModel.Fparcela               := edtParcela.Text;
    ...
    if not chkProvisionavel.Checked  then begin

      tituloPagarModel.FregistroProvisao := '';
    end;


    //setando o contrato caso nao tenha
  pContrato := tituloPagarModel.Fcontrato;

  if uutil.Empty(pContrato) then
  begin
    pContrato := MD5String(tituloPagarModel.FIDCentroCusto);
  end;

//  pContrato := pContrato.Substring(0, 19);
  tituloPagarModel.Fcontrato := pContrato;
...
begin
  ...
  if not (titulo.Fnovo) then  //já existe.. então atualiza
  begin
    try
     sucesso := TTituloPagarDAO.Update(titulo);
     if sucesso then dmConexao.conn.CommitRetaining;
...
procedure TfrmManterTituloPagar.edtContratoChange(Sender: TObject);
var
pContrato :string;
...
begin
 ...
 pContrato :='';
  ...
  if not uutil.Empty(edtContrato.Text) then
  begin

    pContrato := edtContrato.Text;
    //pContrato := pContrato.Substring(0, 19);
  end;

  tituloPagarModel.Fcontrato :=  pContrato;
  edtContrato.Text := pContrato;

end;

procedure TfrmManterTituloPagar.edtParcelaChange(Sender: TObject);
begin
 tituloPagarModel.Fparcela := edtParcela.Text;
end;
...
//  vk_f4: dxBtnNew.Click;
    ...
    vk_f5:
      ...
      try
        ...
        on e: Exception do
          ...
          'Contate o suporte. Informe erro: MANTER_HIST ');
      end;
    ...
    vk_f6:
      ...
      try
        ...
        on e: Exception do
          PempecMsg.MsgInformation(e.Message + sLineBreak +
          'Contate o suporte. Informe erro: MANTER_CEDENTE ');
      end;
...
begin
 ...
 if tituloPagarModel.Fnovo then begin
  ...
  begin
    ...
    tituloPagarModel.FIDorganizacao := cedenteModel.FIDorganizacao;

    qryBaixas.Close;
    qryBaixas  := TTPBaixaDAO.obterBaixasPorCedente(tituloPagarModel);
    dbgBaixas.DataSource.DataSet := qryBaixas;
    totalBaixa := 0;
    //totalBaixa := TTPBaixaDAO.obterValorPagoPorCedente(tituloPagarModel);

    if not uutil.Empty(cedenteModel.Fnome) then begin
   //   panelBaixas.Caption := cedenteModel.Fnome + ' -  TOTAL PAGO  -  R$ ' +  FormatFloat(',0.00', StrToFloat(CurrToStr(totalBaixa )));
    ...
    begin
      ...
      edtCEDReduz.Text := 'NC';
      dxBtnProvisao.Enabled := False;
      msg := 'Fornecedor sem conta contábil. Não será possível provisionar títulos. ';
      painel.Lines.Add(msg);
  ...
  if not uUtil.Empty(cedenteModel.FIDcartaoCreditoModel) then begin
    ...
    if not uUtil.Empty(cedenteModel.FcartaoCredito.FDiaCompra)   then begin
    diaCompra := StrToInt(cedenteModel.FcartaoCredito.FDiaCompra);
    end;

    if not uUtil.Empty(cedenteModel.FcartaoCredito.FDiaVencimento)   then begin
    ...
    vctoCartao := IntToStr(diaVcto) + '/' + IntToStr(MonthOf(IncMonth(Now, 0))) + '/' + IntToStr(yearOf(Now));
      ...
      begin
        ...
        if MonthOf(Now) > 11 then begin

          vctoCartao := IntToStr(diaVcto) + '/' +IntToStr(MonthOf(IncMonth(Now, 1))) + '/' + IntToStr(yearOf(Now)+1);
...
begin
  ...
  indice := 1;
    ...
    frameResponsavel1.cbbcombo.ItemIndex := indice;

  if not uUtil.Empty(responsavelModel.FID) then
  begin

    indice :=  obterIndiceLista(responsavelModel.FID,FSlistaResponsavel);
...
begin
   ...
   StatusBotoes(0);
    ...
    if status.FID.Equals('ABERTO') then begin
            ...
            dxBtnRecibo.Enabled     := False; //Não permite emitir recibo com TP aberto
...
begin
       ...
       if frameCedente1.cbbcombo.ItemIndex > 0 then
      begin
        ...
        begin
          ...
          begin
            cedenteModel.FStatus := 'ATIVO';
            TCedenteDAO.Update(cedenteModel);
          end;
...
begin
  ...
  begin
    frameCentroCusto1.cbbcombo.ItemIndex := 0;
    msg := 'O valor do título não pode ser 0 ';
    PempecMsg.MsgInformation(msg);
  end;
...

 {
  ...
  begin
   msg :='Você precisa selecionar um centro de Custos. ';
   PempecMsg.MsgInformation(msg);
...
if frameResponsavel1.cbbcombo.ItemIndex > 0 then begin
 ...
 {
  //  else begin PempecMsg.MsgInformation('Selecione um responsável.'); end;
...
posicao :=0;
 ...
 // limparPanel;
 dxRibbon1Tab1.Caption := '... ' ;
 nDoc := frameTP1.cbbTP.Text;
  if not uutil.Empty(nDoc) then
  begin

    posicao := obterIndiceListaPorDOC(nDoc);

    frameTP1.cbbTP.ItemIndex := 0;

    if posicao > 0 then
    begin
      frameTP1.cbbTP.ItemIndex := posicao;
      sucesso := True;
    end;
  ...
  begin
    PempecMsg.MsgError('Digite um número de documento. ');
  end;
  ...
  BtnGerarDOC.Enabled := True;
        if sucesso then
        begin //foi informado um numero de documento
         ...
         //titulo veio da consulta
          ...
          if not uutil.Empty(tituloPagarModel.FID) then
          begin
            Self.Caption := 'Manutenção de título a pagar  NUM.  : ' + tituloPagarModel.FnumeroDocumento + ' FORN. : ' + tituloPagarModel.FCedente.Fnome;
            dxRibbon1Tab1.Caption := ' DOC NUM.  : ' + tituloPagarModel.FnumeroDocumento + ' FORN. : ' + tituloPagarModel.FCedente.Fnome;
            ...
            //dados da baixa. Caso exista.

              if ((tituloPagarModel.FIDTipoStatus.Equals('QUITADO')) or (tituloPagarModel.FIDTipoStatus.Equals('PARCIAL'))) then
              begin
                ...
                dbgBaixas.DataSource.DataSet := TTPBaixaDAO.obterBaixasPorCedente(tituloPagarModel);



                if not uutil.Empty(tpBaixaModel.FID) then begin

                  edtValorPago.Value := tpBaixaModel.FvalorPago;

                  if tpBaixaModel.FvalorPago = 0 then begin

                    PempecMsg.MsgError(' A baixa do título está incorreta. Verifique.');
          ...
          begin
            dxRibbon1Tab1.Caption := ' ';
            Self.Caption := '';
            PempecMsg.MsgInformation('Título não localizado... ');
            FreeAndNil(tituloPagarModel)
          end;
          BtnGerarDOC.Caption := 'Consultar';
  end
  ...
  begin
     dxRibbon1Tab1.Caption := ' ';
     Self.Caption := '';
    PempecMsg.MsgError('Título não encontrado. Verifique o número do documento. ');

  end;

  msgStatusBar(2, 'ATENCÂO :' );
  msgStatusBar(3, 'CTRL + F12 carrega todos os títulos - F5 manutenção de histórico.' );
...
begin
 chkProvisionavel.Checked := False;
   ...
   if not uUtil.Empty(titulo.FregistroProvisao) then begin
      chkProvisionavel.Checked := True;
   end ;
   ...
   Label8.Caption           := 'Valor a pagar';
   painel.Lines.Add('Identificador interno: '  + titulo.FID);
   ...
   Label8.Caption           := 'Valor principal';
...
begin
  bloqueiaCampos(Self, true);
   ...
   edtUltimoUpdate.Text        := DateToStr(titulo.FdataUltimaAtualizacao);
   edtDataAtualiza.Text :=    edtUltimoUpdate.Text;
...
begin
  ...
  if Assigned(dbgBaixas.DataSource.DataSet)  then begin
     dbgBaixas.DataSource.DataSet.Close;
  end;
...
function TfrmManterTituloPagar.obterIndiceListaPorDOC(pDOC: string): Integer;
//pega o index do combo tendo o numero do documento
var
  I: Integer;
begin
 Result :=0;

  for I := 0 to frameTP1.cbbTP.Items.Count -1 do begin
    if(frameTP1.cbbTP.Items[I].Equals(pDOC)) then begin
       Result := I;
       Break;
    end;
...
begin
  ...
  begin
    if Form.Components[I].ClassType = TEdit then
      ...
      begin
        ...
        TEdit(Form.Components[I]).TextHint := 'Campo Obrigatório';
  ...
  if frmTipoCobranca1.cbbTipoCobranca.ItemIndex < 1 then begin
   ...
   if frmLocalPagto1.cbbcombo.ItemIndex < 1 then begin
      ...
      begin
        ...
        begin  //só bloqueia se for titulo provisionado
          sucesso := False;

          msgStatusBar(1, 'Verifique a conta contábil do cedente/fornecedor.');
          frameCedente1.cbbcombo.Enabled := True;
          frameCedente1.cbbcombo.SetFocus;
          msg := 'Fornecedor sem conta contábil. Favor corrigir o problema. ';
          PempecMsg.MsgInformation(msg);
          painel.Lines.Add(msg);
  ...
  begin
    ...
    msg := 'Histórico sem conta contábil. Favor corrigir o problema.'+ #13 + ' Aperte F5 para corrigir o histórico. ';
    PempecMsg.MsgInformation(msg);
  ...
  if edtDifRateioCC.Value <>0  then begin
        sucesso := False;
    msg := 'O valor do Rateio de Centro de Custos está incorreto. ';
    PempecMsg.MsgInformation(msg);

  end;


  if edtDifHistorico.Value <>0  then begin
        sucesso := False;
    msg := 'O valor do Rateio de Históricos está incorreto. ';
  ...
  //verificando se existem baixas do mesmo titulo

    if uutil.Empty(tituloPagarModel.Fcontrato)  then begin

     tituloPagarModel.Fcontrato :=  MD5String(tituloPagarModel.FIDCentroCusto);

    end;


    totalPago := totalBaixaPorContrato(pIDorganizacao, tituloPagarModel.FIDCedente  ,tituloPagarModel.Fcontrato, tituloPagarModel.Fparcela);

    if totalPago > 0  then begin
        sucesso := False;

        msg := ' ATENÇÃO :  O contrato ou a parcela possui quitação total. Por favor, verifique a parcela ou o número do contrato.  ';
