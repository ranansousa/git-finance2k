...
type
  TdmExportaFinance = class(TDataModule)
    ...
    qryObterParcelas: TFDQuery;
    qryObterValorProvisionado: TFDQuery;
    qryObterPlanoHistoricoMega: TFDQuery;
    qryObterHistoricoPadraoMega: TFDQuery;
    qryListaHistoricos: TFDQuery;
  ...
  private
    ...
    function obterQtdParcelasProvisionados (pIdOrganizacao, pTable, pRegistroProvisao: String): Integer;
    function obterValorProvisionados (pIdOrganizacao, pTable, pRegistroProvisao: String): Currency;
...
//  function obterTPProvisionados(pIdOrganizacao,pIdStatus :String ; pDataInicial, pDataFinal:TDate;pProvisao :Integer): Boolean;
    ...
    function updateIDLoteContabilGeneric(pTable,pTipo,pIdOrganizacao,pIdRegistro, pIdLote, registroProvisao :string): Boolean;
    ...
    function gravarLoteContabil(pIdLote, pIdOrganizacao, pLote, pDataInicial, pDataFinal: string; pLista :TListaLancamentos; pTable, pTipo :string; qtdRegistros :Integer; valorDebito, valorCredito : Currency): Boolean;
...
//LOTE CONTABIL
 ...
 //genericos
      ...
      function validarCodigosHistoricos(pIdOrganizacao : string;  var loMostraErros : TFMostraErros  ) :boolean;




 //codigo de historico padrao

 function obterCodigoHistoricoPadraoMega(pCODIGO :Integer) : Boolean;
 function obterPlanoHistorioMega():Boolean;



 //HISTORICOS
 function obterlistaHistoricos (pIdOrganizacao: String): Boolean;
...
function TdmExportaFinance.obterCodigoHistoricoPadraoMega( pCODIGO: Integer): Boolean;
var
sucesso : Boolean;
begin
sucesso := False;
     //recebe um codigo (INT) e retorna True se encontrou no MEGA

      dmConexao.conectarMega;
      //verificar no MEGA se existe a conta com base no codido reduzido
      try
        qryObterHistoricoPadraoMega.Connection := dmConexao.ConnMega;
        qryObterHistoricoPadraoMega.Close;
        qryObterHistoricoPadraoMega.ParamByName('PCODIGO').AsInteger := pCODIGO;
        qryObterHistoricoPadraoMega.Open;

         if not qryObterHistoricoPadraoMega.IsEmpty then
          sucesso := True;

      except

        raise(Exception).Create('Problemas ao tentar obter o código do historico padrão ');
...
function TdmExportaFinance.obterlistaHistoricos(pIdOrganizacao: String): Boolean;
var
sucesso : Boolean;
begin
sucesso := False;
  ...
  try
    qryListaHistoricos.Connection := dmConexao.conn;
    qryListaHistoricos.Close;
    qryListaHistoricos.ParamByName('PIDORGANIZACAO').AsString := pIdOrganizacao ;
    qryListaHistoricos.Open;

    if not qryListaHistoricos.IsEmpty then
      sucesso := True;

  except

    raise(Exception).Create('Problemas ao tentar obter a lista de historicos Sistema Financeiro ');
...
function TdmExportaFinance.obterPlanoHistorioMega: Boolean;
var
sucesso : Boolean;
begin
sucesso := False;
     //recebe o conjunto de codigos padroes registrados no SIst Financeiro
...
function TdmExportaFinance.obterQtdParcelasProvisionados(pIdOrganizacao, pTable,   pRegistroProvisao: String): Integer;
  var
  qtdResult :Integer;
  sqlCmd :string;
...
qtdResult :=0;
dmConexao.conectarBanco;
   sqlCmd := ' SELECT  COUNT(TP.REGISTRO_PROVISAO) AS PARCELA '+
             ' FROM TITULO_PAGAR TP '+
             ' WHERE (TP.ID_ORGANIZACAO = :PIDORGANIZACAO) '+
             ' AND   (TP.REGISTRO_PROVISAO = :PREGISTRO  ) '+
             ' AND   (TP.ID_TIPO_STATUS <> ''EXCLUIDO'') '+
             ' AND   (TP.ID_LOTE_TPB IS NULL) ';
 ...
 try
   qryObterParcelas.Close;
   qryObterParcelas.Connection := dmConexao.conn;
   qryObterParcelas.SQL.Clear;
   qryObterParcelas.SQL.Add(sqlCmd);

   qryObterParcelas.ParamByName('PIDORGANIZACAO').AsString := pIdOrganizacao;
   qryObterParcelas.ParamByName('PREGISTRO').AsString := pRegistroProvisao;
   qryObterParcelas.Open;

   qtdResult :=    qryObterParcelas.FieldByName('PARCELA').AsInteger;
 except
  raise Exception.Create('Erro ao obter a quantidade de parcelas do titulo ' );
 end;
...
function TdmExportaFinance.updateIDLoteContabilGeneric(pTable,pTipo, pIdOrganizacao,  pIdRegistro, pIdLote, registroProvisao: string): Boolean;
var
  comando : string;
begin
  ...
  if (pTable = 'TITULO_PAGAR') then begin //se for titulo pagar provisionado sem baixa
    if pTipo.Equals('TPPROV') then begin
  ...
  if (pTable = 'TITULO_RECEBER') then begin
      if pTipo.Equals('TRPROV') then begin


          if not ( (registroProvisao = '0') or  (registroProvisao = '')  ) then begin

           comando := ' UPDATE ' + pTable + ' SET  ID_LOTE_TRB = ' + QuotedStr(pIdLote) +
                      ' WHERE ( REGISTRO_PROVISAO = ' + QuotedStr(registroProvisao) + ') AND (ID_ORGANIZACAO = ' + QuotedStr(pIdOrganizacao) + ' )';
          end;
...
function TdmExportaFinance.validarCodigosHistoricos(pIdOrganizacao : string; var loMostraErros : TFMostraErros ) :boolean;
var
sucesso : Boolean;
qtd,pCodigo : Integer;
...
begin
 ...
 qtd:=0;
 pCodigo :=0;
  //carrega a lista de codigos do mega
  //carrega a lista de historicos do finance
  //verifica se todos os codigos dos historicos do finance existem no mega
  // retona a tela com os codigos nao encontrados


  if (obterlistaHistoricos(pIdOrganizacao)) then
  begin

    if obterPlanoHistorioMega() then
    begin

      qryListaHistoricos.First;
      loMostraErros.Add('>>>  HISTÓRICOS COM CÓDIGO PADRÃO INCORRETO  <<< ', False, True);
      loMostraErros.Add('', False, True);

      while not qryListaHistoricos.Eof do
      begin

        pCodigo := qryListaHistoricos.FieldByName('CODIGO').AsInteger;

        if not obterCodigoHistoricoPadraoMega(pCodigo) then
        begin
          Inc(qtd);
          loMostraErros.Add(' -> ' + qryListaHistoricos.FieldByName('DESCRICAO').AsString + ' CODIGO : -> ' + IntToStr(pCodigo), False, True);

        end;

        qryListaHistoricos.Next;

      end;

      loMostraErros.Add('  ', False, True);
      loMostraErros.Add(' Tenha certeza que os códigos de histórico padrão que são utilizados, estão cadastrados no sistema contábil. ', False, True);
...
function TdmExportaFinance.gravarLoteContabil(pIdLote, pIdOrganizacao, pLote, pDataInicial, pDataFinal: string; pLista :TListaLancamentos; pTable, pTipo :string; qtdRegistros :Integer; valorDebito, valorCredito : Currency): Boolean;
var
...
//  qtdRegistros := TListaLancamentoCredito(plista.ListaCredito[0]).FListaLancamentoCredito.Count;
  ...
  pUsuario := IntToStr(uutil.TUserAtual.getUserId);
  pStatus := 'EXPORT/AUTO';
  ...
  comando := 'INSERT INTO LOTE_CONTABIL (ID_LOTE_CONTABIL,ID_ORGANIZACAO, LOTE, STATUS, '
           ...
           + 'TIPO_TABLE,TIPO_LANCAMENTO,  QTD_REGISTROS, VALOR_DB, VALOR_CR ) '
           + 'VALUES (:pIdLote, :pIdOrganizacao, :pLote, :pStatus, :pUsuario, '
           + ':pDataInicial,:pDataFinal,:pDataRegistro,:pDataAtualizacao, :pTable, :pTipoLanc, :pQtdRegistros, :pValorDB, :pValorCR )';
  ...
  try
    ...
    qryGravarLoteContabil.ParamByName('pTipoLanc').AsString := pTipo;
        ...
        tamLista := TListaLancamentoCredito(pLista).Count ;   //usado só na barra de progresso
          ...
          for   listaCR := 0 to TListaLancamentoCredito(pLista).Count -1 do begin
            ...
            begin
                  ...
                  updateIDLoteContabilGeneric(pTable, pTipo, pIdOrganizacao, idRegistro, pIdLote,registroProvisao );
...
var
 pCodigoHistorico, pCodigoReduzidoDB, pCodigoReduzidoCR,  aux: Integer;
...
//obtendo lancamentos de lista de creditos
    ...
    begin
     {operações
       transferencias entre contas         transfere do caixa para banco       transfere do banco para caixa         }

      tipoLancamento :=  query.FieldByName('HISTORICO').AsString ;

      pCodigoHistorico  :=query.FieldByName('CDHIST').AsInteger;   //ok em 02/12
      ...
      tempLancamentoCR.SetCodHistorico(pCodigoHistorico);
      ...
      tempLancamentoCR.SetHistorico(query.FieldByName('HISTORICO').AsString);
       ...
       if (tipoLancamento.Equals('TRANSFERENCIA ENTRE CONTAS') ) then begin

       tempLancamentoCR.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoCR.SetHistorico('TRANSF ENTRE CONTAS');

       end;

       if (tipoLancamento.Equals('TRANSFERE BANCO/TESOURARIA') ) then begin

       tempLancamentoCR.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoCR.SetHistorico('TRANSF BCO/CAIXA');

       end;


      if (tipoLancamento.Equals('DEPOSITO TESOURARIA/BANCO') ) then begin

       tempLancamentoCR.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoCR.SetHistorico('TRANSF CAIXA/BCO');
      ...
      tempLancamentoCR.SetExecTrigger('S'); //S sempre
       ...
       if ( pCodigoReduzidoCR = 0)  then begin   //não pode ser zero
        ...
        if ( pCodigoHistorico = 0)  then begin   //não pode ser zero
          raise Exception.Create('Código de historico padrão incorreto  '  + tempLancamentoCR.Complemento );
       end;
    ...
    //linha do debito
      ...
      tempLancamentoDB.SetData(query.FieldByName('DATA').AsDateTime);
     {
      ...
      tempLancamentoDB.SetCodHistorico(pCodigoHistorico);
      tempLancamentoDB.SetComplemento(query.FieldByName('COMPL').AsString + ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
      tempLancamentoDB.SetHistorico(query.FieldByName('HISTORICO').AsString);

       if (tipoLancamento.Equals('TRANSFERENCIA ENTRE CONTAS') ) then begin

       tempLancamentoDB.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoDB.SetHistorico('TRANSF ENTRE CONTAS');
       ...
       tempLancamentoDB.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoDB.SetHistorico('TRANSF BCO/CAIXA');
      ...
      if (tipoLancamento.Equals('DEPOSITO TESOURARIA/BANCO') ) then begin
       ...
       tempLancamentoDB.SetComplemento( ' ' + query.FieldByName('CTAORIGEM').AsString + ' --> ' + query.FieldByName('CTADESTINO').AsString );
       tempLancamentoDB.SetHistorico('TRANSF CAIXA/BCO');
      ...
      tempLancamentoDB.SetExecTrigger('S'); //S sempre
       ...
       if ( pCodigoReduzidoDB = 0)  then begin   //não pode ser zero
          raise Exception.Create('Erro ao obter o código reduzido.  Valor 0 '  + tempLancamentoDB.Complemento );
       end;
...
//  loMostraErros: TFMostraErros;
 pRegistroProvisao, pID, pID_ORG, cmd :string;
  ...
  CDHIST_DB, CDHIST,CDREDUZDEB, CDREDUZCRE: Integer;
    ...
    while (not query.Eof) do
       begin
          ...
          pCodigoReduzidoDB :=0;
             ...
             qtdParcelas       := obterQtdParcelasProvisionados(pID_ORG,'TP', pRegistroProvisao); // query.FieldByName('PARCELA').AsInteger;
             fData             := query.FieldByName('DATA_EMISSAO').AsDateTime;
             CDHIST_DB         := query.FieldByName('CDHIST').AsInteger;
            ...
            try
                ...
                if not qryTPPROVCR.IsEmpty then begin
                  ...
                  qryTPPROVCR.First;
                   ...
                   begin
                       pCodigoReduzidoCR := 0;
                       //   VALOR       := (qtdParcelas * (qryTPPROVCR.FieldByName('VALOR').AsCurrency));
                          VALOR       := obterValorProvisionados(pID_ORG,'TP', pRegistroProvisao);
                          ...
                          IDENTIFCRE  :='TP#'+ qryTPPROVCR.FieldByName('IDENTIFCRE').AsString;
...
//                          CDHIST      := qryTPPROVCR.FieldByName('CDHIST').AsInteger;
                          ...
                          CONTA_CRE   := qryTPPROVCR.FieldByName('CONTA_CRE').AsString;
                           ...
                           if HISTORICO.Length >50 then begin
                             ...
                             HISTORICO   := qryTPPROVCR.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPPROVCR.FieldByName('COMPL').AsString;
                        ...
                        {  
                          ...
                          tempLancamentoCR.SetDgCodReduzCre(DGREDUZCRE);  }
                           ...
                           pCodigoReduzidoCR := CDREDUZCRE;   //erro aqui ranan

                        {   if ( pCodigoReduzidoCR = 0)  then begin   //não pode ser zero
                             // raise Exception.Create('Erro ao obter o código reduzido.  '  );
                                    loMostraErros.Add(' ---------------------------------------------------', True, True);
                                    loMostraErros.Add('Lançamento(s) COM ERRO NO CODIDGO REDUZIDO' , False, True);
                                    loMostraErros.Add( 'DOC         '+  tempLancamentoCR.FncIdentificacao, False, True);
                                    loMostraErros.Add( 'DESCRICAO   ' + tempLancamentoCR.Complemento, False, True);
                                    loMostraErros.Add( 'HISTORICO   ' + tempLancamentoCR.Historico, False, True);
                                    loMostraErros.Add( 'CONTA CR    ' + CONTA_CRE, False, True);
                                    loMostraErros.Add( 'CODIGO NÃO LOCALIZADO NO PLANO CONTÁBIL :  '+  IntToStr(tempLancamentoCR.CodReduzCre), False, True);
                                    loMostraErros.Add(' ---------------------------------------------------', True, True);
                                    loMostraErros.Add(' O lote '+ IntToStr(plote) +  ' não será exportado. ' , True, True);
                           end; }
    ...
    //linha do debito
         ...
         if not qryTPPROVDB.IsEmpty then begin
            ...
            begin
              ...
              //VER COMO FICA NO RATEIO DO PROVISIONADO
              //TALVEZ PRECISE REFAZER OS CALCULOS
               //nao funciona com TPPROV e varios historicos

               // RSS 28/11                 VALOR       := qryTPPROVDB.FieldByName('VALOR').AsCurrency;
               //  VALOR := (qtdParcelas * (qryTPPROVDB.FieldByName('VALOR').AsCurrency));
                 VALOR       := obterValorProvisionados(pID_ORG,'TP', pRegistroProvisao);
                ...
                if HISTORICO.Length > 50 then
                begin

                  HISTORICO := qryTPPROVDB.FieldByName('HISTORICO').AsString;
                  COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTPPROVDB.FieldByName('COMPL').AsString;
                  HISTORICO := Copy(HISTORICO, 0, 48) + '#';
             ...
             {  if ( pCodigoReduzidoDB = 0)  then begin   //não pode ser zero
                 // raise Exception.Create('Erro ao obter o código reduzido.  '  + tempLancamentoDB.Complemento );
                  loMostraErros.Add('Lançamento(s) COM ERRO NO CODIDGO REDUZIDO' , False, True);
                                    loMostraErros.Add(' ---------------------------------------------------', True, True);
                                    loMostraErros.Add( 'DOC         '+  tempLancamentoDB.FncIdentificacao, False, True);
                                    loMostraErros.Add( 'DESCRICAO   ' + tempLancamentoDB.Complemento, False, True);
                                    loMostraErros.Add( 'HISTORICO   ' + tempLancamentoDB.Historico, False, True);
                                    loMostraErros.Add( 'CONTA DB   ' + CONTA_DEB, False, True);
                                    loMostraErros.Add( 'CODIGO NÃO LOCALIZADO NO PLANO CONTÁBIL :  '+  IntToStr(tempLancamentoDB.CodReduzDeb), False, True);
                                    loMostraErros.Add(' ---------------------------------------------------', True, True);
                                    loMostraErros.Add(' O lote '+ IntToStr(plote) +  ' não será exportado. ' , True, True);
               end;    }
    ...
    begin
      FsListaLancamentos.listaDebito.Clear;
      FsListaLancamentos.listaCredito.Clear;

      //FreeAndNil(FsListaLancamentos);

      loMostraErros.ShowModal;
      raise Exception.Create('Ocorreram problemas ao tentar exportar. ');
    end;
       ...
       //caso do titulo a pagar
          ...
          if pTable.Equals('TITULO_PAGAR') then begin
             ...
             comando :='SELECT Sum(' + pCampoSum +') as VALOR_DEBITO ' +
                     ...
                     //  'AND TB.ID_LOTE_CONTABIL IS NULL '; retirado em 28/07 par igualar com exportar
...
pIDBAIXA, pRegistroProvisao, pID, pID_ORG, cmd :string;
  VALOR: Currency; fData: TDate;
   CDHIST_CR, CDHIST_DB, CDHIST,CDREDUZDEB, CDREDUZCRE: Integer;
  IDENTIFCRE, ID,HISTORICO, COMPL,DGREDUZCRE, DGCRE,CONTA_CRE :string;
 idStatus, pFormaPagto, auxCD, IDENTIFDEB, DGREDUZDEB ,DGDEB, CONTA_DEB, nDOC :string;
...
valorAC, valorDE ,valorHistorico,  VALOR_PAGO, chkValorCR, chkValorDB : Currency;
begin
//rss 25/11
  try
    FsListaLancamentos := TListaLancamentos.Create;
    FsListaCredito := TListaLancamentoCredito.Create;
    FsListaDebito := TListaLancamentoDebito.Create;
     CDHIST_CR :=0; CDHIST_DB :=0;

    query.First; //contem alguns dados do TITULO
    //precisa consultar os creditos e debitos
    chkValorCR :=0; chkValorDB :=0;
    valorAC :=0; valorDE :=0;
    while (not query.Eof) do
       begin
           ...
           valorAC :=0; valorDE :=0;
            VALOR :=0; VALOR_PAGO :=0;
             ...
             idStatus          := query.FieldByName('STATUS').AsString;
             VALOR             := query.FieldByName('VALOR_NOMINAL').AsCurrency;
             VALOR_PAGO        := query.FieldByName('VALOR_PAGO').AsCurrency;

             pID_ORG           := query.FieldByName('ID_ORGANIZACAO').AsString;
            // qtdParcelas       := query.FieldByName('QTD').AsInteger;
             fData             := query.FieldByName('DATA_PAGAMENTO').AsDateTime;
             pRegistroProvisao := query.FieldByName('REGISTRO_PROVISAO').AsString;
             IDENTIFCRE        := 'TR#'+ query.FieldByName('NUMERO_DOCUMENTO').AsString;
             IDENTIFDEB        := 'TR#'+query.FieldByName('NUMERO_DOCUMENTO').AsString;
             CDHIST            := query.FieldByName('CDHIST').AsInteger;
             CDHIST_DB :=  CDHIST;

            // casos de titulos com pagto parcial
             if idStatus.Equals('PARCIAL')  then begin
              VALOR := VALOR_PAGO;
             end;



          {   if IDENTIFCRE.Equals('50724402')  then begin
                ...
                //conta debito = ID_conta_contabil_credito do TP (Fornecedor)
                  ...
                  //   CDHIST      := query.FieldByName('CDHIST').AsInteger;
                     ...
                     if HISTORICO.Length >50 then begin
                       ...
                       HISTORICO   := query.FieldByName('HISTORICO').AsString ;
                       COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ query.FieldByName('COMPL').AsString;
                       HISTORICO   := Copy(HISTORICO, 0,48)+'#';
                    ...
                    // VALOR := ((VALOR + valorAC) - valorDE);
                     chkValorDB := chkValorDB + VALOR;
                  ...
                  loMostraErros.Add('VALOR PROVISAO  ' +  nDOC + ' VALOR PROV ' +  FormatCurr('###,##0.00', chkValorDB) );
                ...
                if obterTPBHistorico(pID_ORG,pID) then begin
                  //obtendo o rateio de historicos e suas contas contabeis
                   ...
                   while not qryTPBHistorico.Eof do begin
                       ...
                       if  CDHIST = 0 then begin

                       loMostraErros.Add('ERRO CODIGO HISTORICO  TP ' + NDOC );
                     ...
                     if HISTORICO.Length >50 then begin
                       ...
                       HISTORICO   := qryTPBHistorico.FieldByName('HISTORICO').AsString ;
                       COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBHistorico.FieldByName('COMPL').AsString;
                       HISTORICO   := Copy(HISTORICO, 0,48)+'#';
                       ...
                       chkValorDB := chkValorDB + VALOR;
                      // loMostraErros.Add('TOTAL DO RATEIO HISTORICO ' +  FormatCurr('###,##0.00', VALOR) );

                        valorHistorico := valorHistorico + VALOR;

                       // VALOR = valor do titulo + acrescimos - deducoes
                    ...
                    loMostraErros.Add('HISTORICO ACUMULADO  ' +  HISTORICO + ' VALOR ' +  FormatCurr('###,##0.00', chkValorDB) );
                        ...
                        //  CDHIST      := qryTPBCaixa.FieldByName('CDHIST').AsInteger;
                          ...
                          CONTA_CRE   := qryTPBCaixa.FieldByName('CONTA_CRE').AsString;
                           ...
                           if HISTORICO.Length >50 then begin
                             ...
                             HISTORICO   := qryTPBCaixa.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBCaixa.FieldByName('COMPL').AsString;
                             HISTORICO   := Copy(HISTORICO, 0,48)+'#';
                          ...
                          chkValorCR := chkValorCR + VALOR;
                          loMostraErros.Add('TOTAL DO caixa ' +  FormatCurr('###,##0.00', VALOR) );
                        ...
                        //  CDHIST      := qryTPBCheque.FieldByName('CDHIST').AsInteger;
                          ...
                          CONTA_CRE   := qryTPBCheque.FieldByName('CONTA_CRE').AsString;
                           ...
                           if HISTORICO.Length >50 then begin
                             ...
                             HISTORICO   := qryTPBCheque.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBCheque.FieldByName('COMPL').AsString;
                             HISTORICO   := Copy(HISTORICO, 0,48)+'#';
                          ...
                          chkValorCR := chkValorCR + VALOR;
                          loMostraErros.Add('TOTAL DO CHEQUE ' +  FormatCurr('###,##0.00', VALOR) );
                      ...
                      while not qryTPBInternet.Eof do begin
                          ...
                          CONTA_CRE   := qryTPBInternet.FieldByName('CONTA_CRE').AsString;
                           ...
                           if HISTORICO.Length >50 then begin
                             ...
                             HISTORICO   := qryTPBInternet.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBInternet.FieldByName('COMPL').AsString;
                             HISTORICO   := Copy(HISTORICO, 0,48)+'#';
                          ...
                          loMostraErros.Add('PAGTO ONLINE   ' +  HISTORICO + ' VALOR ' +  FormatCurr('###,##0.00', VALOR) );
                           chkValorCR := chkValorCR + VALOR;
                          loMostraErros.Add('TOTAL DO ONLINE ' +  FormatCurr('###,##0.00', VALOR) );
                ...
                // VERIFICAR SE EXISTEM LANCAMENTOS EM ACRESCIMOS                 //
                 if (obterTPBAC(pID_ORG,pIDBAIXA)) then begin
                  //existem acrescimos  do TP
                   valorAC:=0;  VALOR := 0;
                  while not qryTPBAcrescimos.Eof do   begin
                          VALOR :=0;
                           ...
                           if HISTORICO.Length >50 then begin
                             ...
                             HISTORICO   := qryTPBAcrescimos.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBAcrescimos.FieldByName('COMPL').AsString;
                             HISTORICO   := Copy(HISTORICO, 0,48)+'#';

                           end;

                          valorAC := valorAC + VALOR;
                          chkValorDB := chkValorDB + valorAC;
                          FsListaDebito.Adicionar(convertoTPB_PROVToLancamentoDebito(pCodEmpresa,pLote,StrToInt(pAno),
                                                                           CDREDUZDEB,CDHIST,HISTORICO,COMPL,
                                                                           DGREDUZDEB,CONTA_DEB,DGDEB,'1',ID,IDENTIFDEB,VALOR,fData));
                     ...
                     loMostraErros.Add('ACRESCIMOS  ' +  HISTORICO + ' VALOR ' +  FormatCurr('###,##0.00', chkValorDB) );
                   ...
                   //pegando as deducoes
                      ...
                      while not qryTPBDeducao.Eof do begin
                          ...
                          CONTA_CRE   := qryTPBDeducao.FieldByName('CONTA_CRE').AsString;
                           ...
                           if HISTORICO.Length >50 then begin
                             HISTORICO   := qryTPBDeducao.FieldByName('HISTORICO').AsString ;
                             COMPL       := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' '+ qryTPBDeducao.FieldByName('COMPL').AsString;
                             HISTORICO   := Copy(HISTORICO, 0,48)+'#';

                           end;

                          valorDE := valorDE + VALOR;

                          chkValorCR := chkValorCR + valorDE;
                     ...
                     loMostraErros.Add('DEDUÇÃO  ' +  HISTORICO + ' VALOR ' +  FormatCurr('###,##0.00', chkValorCR) );
...
function TdmExportaFinance.obterValorProvisionados(pIdOrganizacao, pTable,  pRegistroProvisao: String): Currency;
  var
  saldo :Currency;
  sqlCmd :string;
...
begin
 ...
 try
   qryObterValorProvisionado.Close;
   qryObterValorProvisionado.Connection := dmConexao.conn;
   qryObterValorProvisionado.SQL.Clear;
   qryObterValorProvisionado.SQL.Add(sqlCmd);
   qryObterValorProvisionado.ParamByName('PIDORGANIZACAO').AsString := pIdOrganizacao;
   qryObterValorProvisionado.ParamByName('PREGISTRO').AsString := pRegistroProvisao;
   qryObterValorProvisionado.Open;

   saldo :=    qryObterValorProvisionado.FieldByName('VALOR').AsCurrency;
  except
  raise Exception.Create('Erro ao obter valor da provisão ' );
 end;
 Result := saldo;
end;
                ...
                //conta debito = ID_conta_contabil_credito do TP (Fornecedor)
                    ...
                    begin
                     ...
                     FsListaCredito.Adicionar(convertoTITULOinLancamentoCredito(pCodEmpresa,pLote,StrToInt(pAno),
                                                                           CDREDUZCRE,CDHIST,HISTORICO,COMPL,
                                                                           DGREDUZCRE,CONTA_CRE,DGCRE,'2',ID,IDENTIFCRE,VALOR,fData));
                    loMostraErros.Add(' CR VALOR ' +  nDOC + ' VALOR  ' +  FormatCurr('###,##0.00', VALOR) ); //ranan
                ...
                if obterTRBHistorico(pID_ORG,pID) then begin
                  //obtendo o rateio de historicos e suas contas contabeis
                   ...
                   while not qryTRBHistorico.Eof do begin
                       ...
                       COMPL       := 'DOC '+ nDoc + ' ' + query.FieldByName('COMPL').AsString;;
                          ...
                          COMPL       := 'DOC '+ nDoc + ' ' +  qryTRBAcrescimos.FieldByName('COMPL').AsString;
                           ...
                           CONTA_CRE   := qryTRBAcrescimos.FieldByName('CONTA_CRE').AsString;
                            ...
                            begin
                              ...
                              HISTORICO := qryTRBAcrescimos.FieldByName('HISTORICO').AsString;
                              COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRBAcrescimos.FieldByName('COMPL').AsString;
                              HISTORICO := Copy(HISTORICO, 0, 48) + '#';
                      ...
                      while not qryTRBDeducao.Eof do begin
                          ...
                          CONTA_DEB   := qryTRBDeducao.FieldByName('CONTA_DEB').AsString;
                            ...
                            begin
                              ...
                              HISTORICO := qryTRBDeducao.FieldByName('HISTORICO').AsString;
                              COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRBDeducao.FieldByName('COMPL').AsString;
                              HISTORICO := Copy(HISTORICO, 0, 48) + '#';
 ...
 //                         CDHIST      := qryTRBCaixa.FieldByName('CDHIST').AsInteger;
                          ...
                          CONTA_DEB   := qryTRBCaixa.FieldByName('CONTA_DEB').AsString;
                            ...
                            begin
                              ...
                              HISTORICO := qryTRBCaixa.FieldByName('HISTORICO').AsString;
                              COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRBCaixa.FieldByName('COMPL').AsString;
                              HISTORICO := Copy(HISTORICO, 0, 48) + '#';
                           ...
                           if auxValorTP <> VALOR then begin
                            ...
                            if VALOR = 0 then begin
                             ...
                             loMostraErros.Add('DB VALOR '  +  nDOC + ' VALOR  ' +  FormatCurr('###,##0.00', VALOR) ); //ranan
                             FsListaDebito.Adicionar( convertoTITULOinLancamentoDebito(pCodEmpresa,pLote,StrToInt(pAno),
                                                                               CDREDUZDEB,CDHIST,HISTORICO,COMPL,
                                                                               DGREDUZDEB,CONTA_DEB,DGDEB,'1',ID,IDENTIFDEB,VALOR,fData));
                            end;
                          ...
                          CONTA_DEB   := qryTRBCheque.FieldByName('CONTA_DEB').AsString;
                           ...
                           if HISTORICO.Length > 50 then
                            begin
                              ...
                              HISTORICO := qryTRBCheque.FieldByName('HISTORICO').AsString;
                              COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRBCheque.FieldByName('COMPL').AsString;
                              HISTORICO := Copy(HISTORICO, 0, 48) + '#';
                  ...
                  try
                    ...
                    if (obterTRBINTERNET(pID_ORG,pID)) then begin
                      ...
                      while not qryTRBInternet.Eof do begin
                          ...
                          HISTORICO   := qryTRBInternet.FieldByName('HISTORICO').AsString + ' ' +  qryTRBInternet.FieldByName('DESCTADEBITO').AsString;
                          COMPL       := 'DOC '+ nDoc ;
                          ...
                          if HISTORICO.Length > 50 then
                            begin
                              ...
                              COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRBInternet.FieldByName('COMPL').AsString;
                              HISTORICO := Copy(HISTORICO, 0, 48) + '#';
                   ...
                   begin
                         //preencher as variaveis aqui
                          ...
                          begin
                            ...
                            HISTORICO := qryTRCR.FieldByName('HISTORICO').AsString;
                            COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRCR.FieldByName('COMPL').AsString;
                            HISTORICO := Copy(HISTORICO, 0, 48) + '#';
         ...
         if not qryTRDB.IsEmpty then begin
            ...
            begin
                          ...
                          begin
                            ...
                            HISTORICO := qryTRDB.FieldByName('HISTORICO').AsString;
                            COMPL := Copy(HISTORICO, 50, (HISTORICO.Length)) + ' ' + qryTRDB.FieldByName('COMPL').AsString;
                            HISTORICO := Copy(HISTORICO, 0, 48) + '#';
