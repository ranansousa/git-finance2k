...
uses
  ...
  Vcl.ExtCtrls, RxCtrls, Data.DB, Vcl.Grids, Vcl.DBGrids, Vcl.Buttons,MDDAO,MDModel,
...
//      framePB1.lblProgressBar.Visible :=False;
  ...
  begin
    PempecMsg.MsgInformation('Não existem contas a serem importadas.');
  end;
  ...
  registraMovimentacao(uutil.TOrgAtual.getId,'CONTA_CONTABIL','SINC CONTAS',uutil.TUserAtual.getLogin + ' sincronizou contas', 'SUCESSO');

  Application.ProcessMessages;

end;

procedure TfrmSincronizaMega.registraMovimentacao(pOrg, pTable, pAcao, pDsc, pStatus: string);
begin
