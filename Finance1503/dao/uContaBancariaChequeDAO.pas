...
begin
  ...
  begin
    try
      ...
      raise Exception.Create('Não foi possível compensar o cheque. Problemas CBD.');
    ...
    begin
      ...
      try
        if value.FdataCompensacao < IncYear(Now, -1) then begin
           ...
           raise Exception.Create('Não foi possível compensar o cheque. Problemas com a Data do CBCheque.');
        ...
        raise Exception.Create('Não foi possível compensar o cheque. Problemas com CBCheque.');
...
sucesso := False;
 ...
 try
    ...
    chequeDeletar.FIDusuario       := IntToStr(uutil.TUserAtual.getUserId);
...
var
 ...
 cmdSql : string;
begin
 //ptipo informa se é o registro I de inicial ou F de final
  cmdSql := 'SELECT MIN(CBC.DATA_REGISTRO) AS DATA  FROM CONTA_BANCARIA_CHEQUE CBC WHERE 1=1 '; //não precisa do ID_ORG
...
begin
   ...
   raise Exception.Create('Erro ao obter cheque por NÚMERO');
