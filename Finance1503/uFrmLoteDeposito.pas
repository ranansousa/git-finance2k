...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, cxGraphics, cxControls, udmConexao, uUtil, uDMOrganizacao, System.DateUtils,
  ...
  FireDAC.Comp.DataSet, FireDAC.Comp.Client, dxBarExtItems, Vcl.ExtCtrls;
...
begin
  ...
  begin
    ...
    msgStatusBar(1, 'Lote pronto para exclusão.');
  end
  else
  begin
    dxBtnEstornarLD.Enabled := False;

    msgStatusBar(1, 'Lote vinculado a lançamentos exportados. ');
...
procedure TfrmLoteDeposito.dxBarBtnPesquisaClick(Sender: TObject);
begin
  ...
  qryObterLotes.ParamByName('pDataInicial').AsDate     := dxBarDataInicial.Date;
  qryObterLotes.ParamByName('pDataFinal').AsDate       := dxBarDataFinal.Date;
  qryObterLotes.Open;
...
procedure TfrmLoteDeposito.dxBarDataInicialChange(Sender: TObject);
begin
dxBarBtnPesquisa.Enabled := True;
end;

procedure TfrmLoteDeposito.dxBarDataFinalChange(Sender: TObject);
begin
dxBarBtnPesquisa.Enabled := True;
end;
...
begin
  ...
  begin
    ...
    limparPanel;
    dxBtnEstornarLD.Enabled := False;
    dxBarBtnPesquisa.Click;

    msgStatusBar(1, 'Lote excluído com sucesso.');
  end;
...
begin
 ...
 msgStatusBar(1, 'Tela de exclusão de lote pronta. Defina a data a ser pesquisada e clique no botão Pesquisar. ');
 ...
 qryDeletaLoteDeposito,qryUpdateTRBC, qryDeletaTD, qryDeletaCBC :TFDQuery;
  ...
  try
    ...
    try
      ...
      raise Exception.Create('Erro ao tentar DELETAR o lote depósito');
    end;
