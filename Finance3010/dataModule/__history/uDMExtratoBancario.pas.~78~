     ...
     if qryCBC.RecordCount >0  then begin
        qryCBC.First;
         while  (not qryCBC.Eof) do begin
          ...
          pDetalhe := qryCBC.FieldByName('descricao').AsString + '  ID ' +
                      qryCBC.FieldByName('identificacao').AsString + ' ' +
                      qryCBC.FieldByName('NUMERO_DOCUMENTO').AsString + ' ' +
                      qryCBC.FieldByName('DSC_TITULO').AsString + ' ' +
                      qryCBC.FieldByName('TIPO_OPERACAO').AsString + ' - ' ;
      ...
      if qryCBD.RecordCount >0  then begin
        qryCBD.First;
         while  (not qryCBD.Eof) do begin
          ...
          pDetalhe := qryCBD.FieldByName('descricao').AsString + '  ID ' +
                      qryCBD.FieldByName('identificacao').AsString + ' ' +
                      qryCBD.FieldByName('DSC_TITULO').AsString + ' ' +
                      qryCBD.FieldByName('TIPO_OPERACAO').AsString + ' CH  ' ;

             if (not uutil.Empty (cheque)) then begin
             pDetalhe := qryCBD.FieldByName('descricao').AsString + '  TP  ' +
                         qryCBD.FieldByName('NUMERO_DOCUMENTO').AsString + ' ' +
                         cheque + ' CTA  ' + qryCBD.FieldByName('CONTA_BANCO').AsString + ' - ';
             end;
