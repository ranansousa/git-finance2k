...
begin
  ...
  begin
    ...
    idEmpresaSistemaContabil := StrToInt(TOrgAtual.getIDSistemaContabil);
    if idEmpresaSistemaContabil < 1 then begin
       try
 ...
 meses := MonthsBetween(pDataInicial, pDataFinal);
   ...
   //Não permite exportação de mais de 31 dias
      ...
      if(MonthsBetween(pDataInicial, pDataFinal)>0) then begin
       ...
       if (idEmpresaSistemaContabil < 0)   then begin
          aux :=False;

          idEmpresaSistemaContabil := dmMegaContabil.obterIDEmpresa(TOrgAtual.getCNPJ);

          if validarIDEmpresaSistemaContabil(idEmpresaSistemaContabil) then begin
             TOrgAtual.setIDSistemaContabil(IntToStr(idEmpresaSistemaContabil));
             aux :=True;
          end;
