...
try
  ...
  if not qryPesquisa.IsEmpty then begin
      ...
      tituloPagar.FIDusuario              := qryPesquisa.FieldByName('ID_USUARIO').AsString;
      tituloPagar.FIDResponsavel          := qryPesquisa.FieldByName('ID_RESPONSAVEL').AsString;
      tituloPagar.FIDloteContabil         := qryPesquisa.FieldByName('ID_LOTE_CONTABIL').AsString;
      tituloPagar.FIDHistorico            := qryPesquisa.FieldByName('ID_HISTORICO').AsString;
      tituloPagar.FIDCedente              := qryPesquisa.FieldByName('ID_CEDENTE').AsString;
      tituloPagar.FIDLoteTPB              := qryPesquisa.FieldByName('ID_LOTE_TPB').AsString;
      tituloPagar.FIDTipoCobranca         := qryPesquisa.FieldByName('ID_TIPO_COBRANCA').AsString;
      tituloPagar.FIDLocalPagamento       := qryPesquisa.FieldByName('ID_LOCAL_PAGAMENTO').AsString;
      tituloPagar.FIDTipoStatus           := qryPesquisa.FieldByName('ID_TIPO_STATUS').AsString;
      tituloPagar.FIDCentroCusto          := qryPesquisa.FieldByName('ID_CENTRO_CUSTO').AsString;
      tituloPagar.FIDNotaFiscalEntrada    := qryPesquisa.FieldByName('ID_NOTA_FISCAL_ENTRADA').AsString;
      tituloPagar.FIDTituloPagarAnterior  := qryPesquisa.FieldByName('ID_TITULO_GERADOR').AsString;
      tituloPagar.FIDLotePagamento        := qryPesquisa.FieldByName('ID_LOTE_PAGAMENTO').AsString;
      tituloPagar.FIDContaContabilDebito  := qryPesquisa.FieldByName('ID_CONTA_CONTABIL_DEBITO').AsString;
      tituloPagar.FIDContaContabilCredito := qryPesquisa.FieldByName('ID_CONTA_CONTABIL_CREDITO').AsString;
      tituloPagar.FIDCBChequeVinculado    := qryPesquisa.FieldByName('ID_CONTA_BANCARIA_CHEQUE').AsString;
      tituloPagar.Fdescricao              := qryPesquisa.FieldByName('DESCRICAO').AsString;
      tituloPagar.Fobservacao             := qryPesquisa.FieldByName('OBSERVACAO').AsString;
      tituloPagar.Fparcela                := qryPesquisa.FieldByName('PARCELA').AsString;
      tituloPagar.Fcontrato               := qryPesquisa.FieldByName('CONTRATO').AsString;
      tituloPagar.FcodigoBarras           := qryPesquisa.FieldByName('CODIGO_BARRAS').AsString;
      tituloPagar.Fcarteira               := qryPesquisa.FieldByName('CARTEIRA').AsString;
      tituloPagar.Fmoeda                  := qryPesquisa.FieldByName('MOEDA').AsString;
      tituloPagar.FregistroProvisao       := qryPesquisa.FieldByName('REGISTRO_PROVISAO').AsString;
      tituloPagar.FnumeroDocumento        := qryPesquisa.FieldByName('NUMERO_DOCUMENTO').AsString;

      tituloPagar.FdataRegistro           := qryPesquisa.FieldByName('DATA_REGISTRO').AsDateTime;
      tituloPagar.FdataUltimaAtualizacao  := qryPesquisa.FieldByName('DATA_ULTIMA_ATUALIZACAO').AsDateTime;
      tituloPagar.FprevisaoCartorio       := qryPesquisa.FieldByName('PREVISAO_CARTORIO').AsDateTime;
      tituloPagar.FdataPagamento          := qryPesquisa.FieldByName('DATA_PAGAMENTO').AsDateTime;
      tituloPagar.FdataVencimento         := qryPesquisa.FieldByName('DATA_VENCIMENTO').AsDateTime;
      tituloPagar.FdataProtocolo          := qryPesquisa.FieldByName('DATA_PROTOCOLO').AsDateTime;
      tituloPagar.FdataEmissao            := qryPesquisa.FieldByName('DATA_EMISSAO').AsDateTime;

      tituloPagar.FvalorNominal           := qryPesquisa.FieldByName('VALOR_NOMINAL').AsCurrency;
      tituloPagar.FvalorAntecipado        := qryPesquisa.FieldByName('VALOR_ANTECIPADO').AsCurrency;
...
cmdSql := ' UPDATE TITULO_PAGAR '+
         ...
         ' WHERE (ID_TITULO_PAGAR = :PIDTITULO_PAGAR ) AND (ID_ORGANIZACAO = :PPIDORGANIZACAO ) ' ;
    ...
    qryUpdate.ParamByName('PIDTITULO_PAGAR').AsString             := titulo.FID;
