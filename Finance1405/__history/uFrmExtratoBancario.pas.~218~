 ...
 //só enviar um tipo.
  ...
  begin
    ...
    if chkPDF.EditValue then begin
     ...
     frmEnviaMail := TfrmEnviaMail.Create(Self, arquivo);
...
//      descConta := frmContaBancaria1.cbbConta.Items[frmContaBancaria1.cbbConta.ItemIndex];
  ...
  begin
   ...
   if  dmExtratoBancario.montarTabelaTemp(IncDay(pDataInicial, -1),'SALDO ANTERIOR DA CONTA ' + descConta , tipoSaldo, vlrSaldoAnterior)  then begin
       dmExtratoBancario.qryExtrato.Close;
       dmExtratoBancario.qryExtrato.Open;
   ...
   end else begin PempecMsg.MsgWarning('Não foi possível exibir o extrato bancário ...'); end;
...
procedure TfrmExtratoBancario.preencheGrid;
begin
