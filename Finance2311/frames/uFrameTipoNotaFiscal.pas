...
begin
  ...
  try
    cmd :=  ' SELECT E.ID_TIPO_NOTA_FISCAL, E.ID_ORGANIZACAO,E.DESCRICAO ' +
            ' FROM TIPO_NOTA_FISCAL E  ' +
  ...
  raise Exception.Create('Erro ao obter lista de TIPO ');
...
begin
  ...
  dmConexao.conectarBanco;
  try
    cmd := ' SELECT TC.ID_TIPO_NOTA_FISCAL, TC.ID_ORGANIZACAO, TC.DESCRICAO FROM TIPO_NOTA_FISCAL TC WHERE (TC.ID_ORGANIZACAO = :PIDORGANIZACAO) AND  (TC.ID_TIPO_NOTA_FISCAL = :PID) ';
    ...
    begin
      ...
      tipo.FID            := qryObter.FieldByName('ID_TIPO_NOTA_FISCAL').AsString;
    ...
    raise Exception.Create('Erro ao obter TIPO NF por ID');
...
qryObterTodos :TFDQuery;
 cmd :string;
begin
  cmd := ' SELECT E.ID_TIPO_NOTA_FISCAL, E.DESCRICAO ' +
         ' FROM TIPO_NOTA_FISCAL E  ' +
         ' WHERE (E.ID_ORGANIZACAO = :PIDORGANIZACAO) ' +
         ' ORDER BY E.DESCRICAO ';
   ...
   if not qryObterTodos.IsEmpty then begin
      ...
      while not qryObterTodos.Eof do
        begin
          ...
          listaID.Add(qryObterTodos.FieldByName('ID_TIPO_NOTA_FISCAL').AsString);
          qryObterTodos.Next;
        end;
      qryObterTodos.Close;
      combo.ItemIndex := 0;
