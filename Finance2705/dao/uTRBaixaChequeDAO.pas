...
qry := TFDQuery.Create(nil);
  ...
  try
    ...
    try
      ...
      if value.FdataDevolucao < value.FdataDeposito then begin

        qry.ParamByName('PDATADEVOLUCAO').Value    := null;

      end;

      if uUtil.Empty(value.FIDloteDeposito) then begin
       qry.ParamByName('PIDLOTE_DEPOSITO').Value    := null;
