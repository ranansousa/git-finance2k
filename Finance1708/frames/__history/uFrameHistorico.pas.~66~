...
begin
 ...
 try
   ...
   if obterHistorico(pIDOrganizacao,pIDHistorico) then begin
      ...
      historico.SetFIdContaContabil(qryObterPorID.FieldByName('ID_CONTA_CONTABIL').AsString);
   end;


  except

    raise(Exception).Create('Não foi possível obter o histórico.');
...
begin
 ...
 raise Exception.Create('Erro ao obter históricos por código padrão');
...
begin
 ...
 try
   ...
   sqlCmd :=  ' SELECT H.ID_HISTORICO, H.ID_ORGANIZACAO,  H.DESCRICAO AS DESCRICAO_HISTORICO, H.TIPO, '+
              ...
              ' FROM HISTORICO H '+
              ' LEFT OUTER JOIN CONTA_CONTABIL CC ON (CC.ID_CONTA_CONTABIL = H.ID_CONTA_CONTABIL) AND (CC.ID_ORGANIZACAO = H.ID_ORGANIZACAO) '+
              ' WHERE (H.ID_ORGANIZACAO = :PIDORGANIZACAO) AND (H.TIPO = :PTIPO) ORDER BY CC.CONTA ';
