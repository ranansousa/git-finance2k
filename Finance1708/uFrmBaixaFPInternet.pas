...
uses
  ...
  uFrameBanco, cxGraphics, cxControls, cxLookAndFeels, cxLookAndFeelPainters,
  dxSkinsCore, dxSkinsDefaultPainters, dxStatusBar, EMsgDlg;
...
begin
  ...
  LimpaTela(Self);
    ...
    if not uutil.Empty(baixaWWW.Fpersonalidade) then begin
    ...
    if not uutil.Empty(baixaWWW.FIDbancoDestino) then begin
    ...
    edtPortador.Text      := uutil.TOrgAtual.getFantasia;
    edtAgencia.Text       := '';
    edtConta.Text         := '';
    edtCNPJCPF.Text       := uutil.TOrgAtual.getCNPJ
    end;
    ...
    frmBanco1.obterTodosPorIndice(pIdOrganizcacao,frmBanco1.cbbBanco,FSListaIDBanco  );
