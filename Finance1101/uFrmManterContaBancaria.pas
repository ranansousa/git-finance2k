...
begin
  ...
  edtAgencia.Text       := (dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('AGENCIA').AsString;
  edtContaBancaria.Text := (dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('CONTA').AsString;
  edtTitular.Text       := (dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('TITULAR').AsString;
  edtSaldoInicial.Text := CurrToStr((dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('SALDO_INICIAL').AsCurrency);
...
begin
 ...
 if not uUtil.Empty(pIdContaBancaria) then begin
        pIdContaBancaria :=  (dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('ID_CONTA_BANCARIA').AsString;
 end;

    pIdBanco   :=FslistaIdBancos[frmBanco1.cbbBanco.ItemIndex];
    pTitular   := UpperCase(edtTitular.Text);
    pAgencia   := edtAgencia.Text;
    pConta     := edtContaBancaria.Text;
    pSaldo     := StrToCurr( edtSaldoInicial.Text);

    if uutil.Empty(pTitular) then begin pTitular := uUtil.TOrgAtual.getFantasia; end;
...
dmConexao.conectarBanco;
 try
    ...
    sqlCmd := ' SELECT CB.ID_CONTA_BANCARIA, CB.ID_BANCO,  CB.ID_ORGANIZACAO, CB.CONTA, ' +
              ...
              ' INNER JOIN BANCO B ON (CB.ID_BANCO = B.ID_BANCO) AND (CB.ID_ORGANIZACAO = B.ID_ORGANIZACAO) '+
              ' INNER JOIN CONTA_CONTABIL CC ON (CB.ID_CONTA_CONTABIL = CC.ID_CONTA_CONTABIL) AND (CB.ID_ORGANIZACAO = CC.ID_ORGANIZACAO) '+
