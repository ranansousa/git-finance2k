...
uses
  ...
  udmConexao, uDMOrganizacao, udmMegaContabil,
  ...
  uFrmAlteraTituloReceber, udmManutencao, uFrmExtratoBancario, uFrmConsolidadoBancos,
  ...
  uFrmTelaPagamentoTR, uFrmTelaPagamentoLoteTP, uOrganizacaoDAO,
  ...
  cxCheckGroup, cxCheckBox, cxButtonEdit, dxSparkline,uFrmTelaSenha,
  cxDropDownEdit,uFrmManterBanco, IdAntiFreeze, EMsgDlg, Vcl.Grids, Vcl.DBGrids;
...
type
  TFrmPrincipal = class(TForm)
    ...
    procedure dxBarBancoClick(Sender: TObject);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            uUtil.TUserAtual.setId(usuario.FID);
  ...
  begin
    ...
    if not (indice = 0 ) then begin
     ...
     usuario.FID :=  StrToInt(pid);
      ...
      on e: Exception do
      PempecMsg.MsgError('CAST DO PID');
...
procedure TFrmPrincipal.dxBarBancoClick(Sender: TObject);
begin

//tela de manutenção de bancos

frmManterBanco := TfrmManterBanco.Create(Self);
frmManterBanco.ShowModal;
FreeAndNil(frmManterBanco);
end;

procedure TFrmPrincipal.dxBarBtnEspelhoCHClick(Sender: TObject);
begin
//tela mostra os dados de um cheque
//memo apresenta o campo observação do cheque
...
//data de inicio e fim para o período
//fazer o FR3
...
numberError := 'SD-BCO-10';
  try
    ...
    frmConsolidadoBancos := TfrmConsolidadoBancos.Create(Self);
    frmConsolidadoBancos.ShowModal;
    FreeAndNil(frmConsolidadoBancos);
...
{$ENDIF}
  ...
  begin
       autorizado := False;
    try
      frmTelaSenha := TfrmTelaSenha.Create(Self, usuario);
      frmTelaSenha.ShowModal;
      autorizado := frmTelaSenha.getAutorizado;
      FreeAndNil(frmTelaSenha);
    except
      on e: Exception do
        PempecMsg.MsgInformation(e.Message + sLineBreak + 'Contate o suporte. Informe erro: ' + numberError);
    end;
  ...
  begin
    ...
    PempecMsg.MsgWarning('Você não tem autorização para usar esta funcionalidade. '+ #13 + ' Entre em contato com o suporte. ' );
  end;
...
begin
 PempecMsg.MsgInformation('Em desenvolvimento. Por favor, aguarde...!');
end;
