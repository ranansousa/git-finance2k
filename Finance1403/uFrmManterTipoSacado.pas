...
type
  TfrmManterTipoSacado = class(TForm)
...
var
  frmManterTipoSacado: TfrmManterTipoSacado;

implementation

{$R *.dfm}

procedure TfrmManterTipoSacado.cxbrdtmPesquisaCurChange(Sender: TObject);
begin
  dbgrd1.DataSource.DataSet.Locate('DESCRICAO',UpperCase(cxbrdtmPesquisa.CurEditValue),[loPartialKey]);
end;

procedure TfrmManterTipoSacado.dbgrd1TitleClick(Column: TColumn);
begin
 (dbgrd1.DataSource.DataSet as TFDQuery).IndexFieldNames :=Column.FieldName;
end;

function TfrmManterTipoSacado.deletar(pIdOrganizacao, pIdValue: string): Boolean;
var cmd :string;
qryDel :TFDQuery;
begin
...
procedure TfrmManterTipoSacado.dsGridDataChange(Sender: TObject; Field: TField);
begin
...
procedure TfrmManterTipoSacado.dxBtnDeletarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoSacado.dxBtnEditarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoSacado.dxBtnFecharClick(Sender: TObject);
begin
 PostMessage(Self.Handle, WM_CLOSE, 0, 0);
end;

procedure TfrmManterTipoSacado.dxBtnNovoClick(Sender: TObject);
begin
...
procedure TfrmManterTipoSacado.dxBtnSalvarClick(Sender: TObject);
var
 pDescricao :string;
begin
...
procedure TfrmManterTipoSacado.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 freeAndNilDM(Self);
 Action :=caFree;
...
procedure TfrmManterTipoSacado.FormCreate(Sender: TObject);
begin
 pTable := 'TIPO_SACADO';
 Self.Caption := pTable;
 dxRibbon1Tab1.Caption := 'Manutenção de tipos de sacados';

 inicializarDM(Self);
 limparPanel;
end;

procedure TfrmManterTipoSacado.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 case key of
...
procedure TfrmManterTipoSacado.freeAndNilDM(Sender: TObject);
begin
...
procedure TfrmManterTipoSacado.inicializarDM(Sender: TObject);
begin
 //nada
...
procedure TfrmManterTipoSacado.limparPanel();
var
  I: Integer;
begin
 limpaStatusBar;
...
procedure TfrmManterTipoSacado.msgStatusBar(pPosicao : Integer; msg :string);
begin
dxStatusBar.Panels[pPosicao].Text := msg;
Application.ProcessMessages;
end;

procedure TfrmManterTipoSacado.preencheGrid(pIdOrganizacao: string);
var
sqlCmd :string;
begin
...
function TfrmManterTipoSacado.liberaDeletar(cmd :string) : Integer;
var
...
function TfrmManterTipoSacado.qtdUso(pIdOrganizacao,  pIdValue: string): Integer;
  var
qryPesquisa, qryTR : TFDQuery;
cmd : string;
qtd :Integer;
...
begin
  ...
  try
      ...
      cmd := ' SELECT FIRST 1  CBD.ID_' + pTable + ' FROM SACADO CBD WHERE (CBD.ID_ORGANIZACAO =:PIDORGANIZACAO) AND (CBD.ID_'+pTable + '  = :PID) ' ;
...
procedure TfrmManterTipoSacado.limpaStatusBar;
begin
dxStatusBar.Panels[0].Text := 'Status ';
dxStatusBar.Panels[1].Text := 'Ativo. Teclas de atalho:  [F2] = Editar  - [F4] = Novo - [F10] = Salvar  ';
end;

function  TfrmManterTipoSacado.salvarContaBancaria(PIDORGANIZACAO, PIDVALUE, PDESCRICAO  :string) :Boolean;
var
cmdSalvar :string;
qrySalvar :TFDQuery;
begin
...
procedure TfrmManterTipoSacado.LimpaTela(Form: TForm);
var
  i: Integer;
begin
...
procedure TfrmManterTipoSacado.controleEdit(Form: TForm; pValue :Boolean);
var
i: Integer;
begin
