...
uses
  ...
  FireDAC.Comp.Client, FireDAC.Comp.DataSet, dxBar, cxClasses, Vcl.StdCtrls, uFrmManterHistorico,
...
begin
 ...
 if modo = 1 then begin
    ...
    if not uUtil.Empty(edtUltimoUpdate.Text) then
    begin
...
begin
  ...
  begin
    ShowMessage('O centro de custo não pode ser utilizado. Possível problema com a consulta . ');
    frameCentroCusto2.cbbcombo.ItemIndex := 0;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('O histórico não pode ser utilizado. Possível problema no campo conta contábil. ');
      frameHistorico2.cbbcombo.ItemIndex := 0;
...
// StatusBotoes(0); retirado em 26/05 pq NF nao está em uso
...
msgStatusBar(3,'Modo de edição. Aba nota fiscal. Para Salvar retorne a aba básica.');
...
begin
  ...
  begin
    permite := False;
     msg :='O título já foi exportado. Primeiro deve remover o lote contábil.';
  ...
  begin
    ...
    begin
      ...
      begin
        msg :='O título será automaticamente provisionado. Deseja cancelar o recebimento? ';
 ...
 if permite then
  begin
    dmConexao.conectarBanco;
      ...
      if not uutil.Empty(trBaixaModel.FID) then begin
        ...
        TTRBaixaACDAO.deletePorTRBaixa(acrescimo);
         ...
         //só deleta se nao tiver deposito
    ...
    begin
      ...
      msg := 'A baixa do título foi cancelada com sucesso. ';
      PempecMsg.MsgInformation(msg);
      limparPanel;
...
procedure TfrmManterTituloReceber.dxBtnPagarClick(Sender: TObject);
 ...
 tituloExist : TTituloReceberModel;

begin
  sucesso := False;
  aux :=0;
  tituloExist := TTituloReceberModel.Create;
    ...
    begin
     try
        ...
        begin
          ...
          tituloExist :=   TTituloReceberDAO.obterPorNumeroDocumento(tituloReceberModel);
            ...
            begin
              aux := 0;
              tituloReceberModel.FnumeroDocumento := dmConexao.obterIdentificador('', pIdOrganizacao, 'TR');
              tituloExist := TTituloReceberDAO.obterPorNumeroDocumento(tituloReceberModel);
        ...
        end else begin sucesso := TTituloReceberDAO.Update(tituloReceberModel); end;
...
begin
   //  pega os dados do form e seta no titulo
    ...
    tituloReceberModel.FIDUsuario             := uutil.TUserAtual.getUserId;
...
begin
   ...
   StatusBotoes(0);
    ...
    if status.FID.Equals('ABERTO') then begin
            ...
            dxBtnRecibo.Enabled     := False; //Não permite emitir recibo com Título aberto
...
//      preencheAbaNotaFiscal(tituloReceberModel.FNotaFiscalEntrada);
    ...
    begin
      FreeAndNil(tituloReceberModel) ;
      msg := 'Não foi possível localiar o titulo. ';
      PempecMsg.MsgInformation(msg );
    end; 
...
function TfrmManterTituloReceber.obterIndiceListaPorDOC(pDOC: string): Integer;
...
begin
 Result :=0;
  ...
  for I := 0 to frameTR1.cbbTR.Items.Count -1 do begin
    if(frameTR1.cbbTR.Items[I].Equals(pDOC)) then begin
       Result := I;
       Break;
    end;
...
posicao :=0;
 ...
 nDoc := frameTR1.cbbTR.Text;
  ...
  begin
    ...
    if posicao > 0 then
    begin
      frameTR1.cbbTR.ItemIndex := posicao;
      sucesso := True;
    end;
...
begin
  ...
  begin
    ...
    begin  //só bloqueia se for titulo provisionado
      ...
      sucesso := False;
      msgStatusBar(1, 'Verifique a conta contábil do sacado/cliente.');
      frameSacado1.cbbcombo.Enabled := True;
      frameSacado1.cbbcombo.SetFocus;
   // PempecMsg.MsgInformation('Cliente sem conta contábil. Favor corrigir o problema. ', mtInformation, [mbOK], 0);
      msg := 'Cliente sem conta contábil. Favor corrigir o problema. ';
      PempecMsg.MsgInformation(msg);
      sucesso := False;
  ...
  begin
    ...
    msg := 'O valor do título não pode ser 0 (zero) ';
    PempecMsg.MsgInformation(msg);
  end;
