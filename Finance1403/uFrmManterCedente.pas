...
type
 ...
 FsListaCartaoCredito, FsListaPersonalidade,  FslistaTipo, FslistaTipoContas,   FsListaIdContas : TStringList;
   FslistaIdBanco,    FsListaIdEstado,FsListaIdCidade,FsListaIdBairro : TStringList;
    ...
    procedure preencheComboTipoConta;
...
begin
  ...
  //   cedenteModel := obterCedentePorId(pIdOrganizacao, pIdCedente); //mudando em 260320
     ...
     if uutil.Empty(cedenteModel.FID) then begin
       PempecMsg.MsgWarning('Não foi possível obter os dados do cedente. Verifique se digitou o nome corretamente...');
     end ;
   //começa
     ...
     if uUtil.Empty(cedenteModel.FStatus)  then begin
     ...
     if not  uutil.Empty(cedenteModel.FtipoConta) then begin

            if cedenteModel.FtipoConta.Equals('C') then
            begin
              cbxTipoConta.ItemIndex := 1;

            end
            else if cedenteModel.FtipoConta.Equals('P') then
            begin

              cbxTipoConta.ItemIndex := 2;
            end
            else
            begin
              cbxTipoConta.ItemIndex := 3;
            end;
     end;
     ...
     preencheComboCartao;
      ...
      Self.dxRibbon1Tab1.Caption := 'Manutenção do cadastro de : ' + UpperCase(edtNomeCedente.Text);
      frmBanco1.cbbBanco.ItemIndex := retornaIndiceCbbBanco(pIdBanco);
    ...
    {   //removido em 10/12 pq o cedente nao trazia os cartoes para vincular
  ...
  if not uUtil.Empty(cedenteModel.FIDcontaContabil) then
  begin
 ...
 try
  //SALVADR ENDERECO
    ...
    // Salvar CONTATO
      ...
      cedente.FtipoConta            := FslistaTipoContas[cbxTipoConta.ItemIndex];
    ...
    begin
      ...
      PempecMsg.MsgInformation(e.Message + sLineBreak + ' Não foi possível salvar o cedente.');
...
procedure TfrmManterCedente.frmBanco1cbbBancoChange(Sender: TObject);
begin
pIdBanco :='';

if frmBanco1.cbbBanco.ItemIndex > 0 then begin

   pIdBanco := FslistaIdBanco[frmBanco1.cbbBanco.ItemIndex];
...
//
 ...
 //botao editar muda
  ...
  frmTipoCedente1.cbbcombo.ItemIndex :=0;
  edtStatus.Text := 'ATIVO';
  Self.dxRibbon1Tab1.Caption := 'Manutenção do cadastro de cedentes/fornecedores.';
...
procedure TfrmManterCedente.preencheComboTipoConta;
begin

  FslistaTipoContas :=TStringList.Create;
  FslistaTipoContas.Clear;
  FslistaTipoContas.Add('O');
  FslistaTipoContas.Add('C');
  FslistaTipoContas.Add('P');
  FslistaTipoContas.Add('O');


  cbxTipoConta.Clear;
  cbxTipoConta.Items.Add(' >>Selecione<<');
  cbxTipoConta.Items.Add('Corrente');
  cbxTipoConta.Items.Add('Poupança');
  cbxTipoConta.Items.Add('Outro');
...
sqlEnd := ' SELECT E.ID_CARTAO_CREDITO, E.ID_ORGANIZACAO, ' +
 ...
 try
  ...
  raise Exception.Create('Erro ao obter CARTÃO DE CRÉDITO ');
        ...
        begin
          ...
          begin
                  ...
                  begin
                    ...
                    msgStatusBar(1, 'O cedente precisa ter conta contábil, endereço e contato. Verifique todas as abas');
                    Break
                  end;
