...
uses
  ...
  FireDAC.Comp.DataSet, uTituloPagarDAO, uTituloPagarModel, uTituloReceberDAO, uTituloReceberModel,
  ...
  FireDAC.Stan.Async, FireDAC.DApt, Vcl.Grids, ENumEd;
...
type
  TfrmManutencao = class(TForm)
    ...
    edtBaixaNumDoc: TEdit;
    Label1: TLabel;
    btnBaixaPesquisa: TBitBtn;
    DBGridBaixasTP: TDBGrid;
    qryObterBaixaDOCTP: TFDQuery;
    dsBaixasTP: TDataSource;
    Label2: TLabel;
    DBGridBXFP: TDBGrid;
    qryBaixaDOCTPFP: TFDQuery;
    dsBaixaDOCTPFP: TDataSource;
    DBGridBXWWW: TDBGrid;
    DBGridBXHistorico: TDBGrid;
    DBGridBXCAIXA: TDBGrid;
    Label3: TLabel;
    dsBaixaTPRateioHST: TDataSource;
    qryObterRateioHistorico: TFDQuery;
    DBGridBXCentroCusto: TDBGrid;
    dsBaixaTPRateioCC: TDataSource;
    Label4: TLabel;
    qryBaixaDOCTPCAIXA: TFDQuery;
    dsBaixaDOCTPCAIXA: TDataSource;
    qryBaixaDOCTPWWW: TFDQuery;
    dsBaixaDOCTPWWW: TDataSource;
    qryBaixaDOCTPCH: TFDQuery;
    dsBaixaDOCTPCH: TDataSource;
    DBGridBXCH: TDBGrid;
    edtFornecedor: TEdit;
    edtDescricao: TEdit;
    edtStatus: TEdit;
    btnDeleteBaixaDocTP: TBitBtn;
    edtTotalRateioCC: TEvNumEdit;
    Label5: TLabel;
    edtDIFRateioCC: TEvNumEdit;
    Label6: TLabel;
    qryObterTRPVazio: TFDQuery;
    ...
    procedure btnBaixaPesquisaClick(Sender: TObject);
    procedure dsBaixasTPDataChange(Sender: TObject; Field: TField);
    procedure btnDeleteBaixaDocTPClick(Sender: TObject);
  ...
  private
    ...
    function ObterTodosTitulosPagar(pIdOrganizacao, pIdStatus  :string): TFDQuery;
    function ObterTodosTitulosPagarDOC(pIdOrganizacao, pDOC: string): TTituloPagarModel;
   ...
   ///LIMPAR TABELA ENDERECO
    ...
    function insereCentroRateioCentroCusto(pIdOrganizacao, pIdTitulo,
      pIdCentroCusto, pTable: string; pValor: Currency): Boolean;
...
sqlAjusta, labelMSG,  idTR, idHistorico, idCentroCusto :string;    //ajusta os rateios de Titulo a REceber
    ...
    framePB1.lblProgressBar.Caption := 'Existem ' + IntToStr(qryObterTodosTRPeriodo.RecordCount) + ' a serem ajustados ' ;
      ...
      try
        ...
        while not (qryObterTodosTRPeriodo.Eof) do begin
          ...
          idCentroCusto := qryObterTodosTRPeriodo.FieldByName('ID_CENTRO_CUSTO').AsString;

          lbl4.Caption := 'Registro atual Nº ' + IntToStr(qryObterTodosTRPeriodo.RecNo) + #13 +
                          'DOC ' +  qryObterTodosTRPeriodo.FieldByName('NUMERO_DOCUMENTO').AsString;
          Application.ProcessMessages;
             ...
             // PARTE DO AJUSTE DO RATEIO DE HISTORICOS
                    ...
                    if (qtdHistorico = 1) then begin
                     ...
                     if (alterarHistorico)  then begin
                      sqlAjusta := ' UPDATE titulo_receber_historico TPH ' +
                       ...
                       qryAjustaHistorico.ParamByName('PVALOR').AsCurrency :=(pValor);
              ...
              if qtdCC = 0 then begin
                ...
                insereCentroRateioCentroCusto(pIdOrganizacao,idTR,idCentroCusto,'TITULO_RECEBER',pValor);
                    ...
                    if (qtdCC = 1) then begin
                     ...
                     if (alterarCC)  then begin
                      ...
                      sqlAjusta := ' UPDATE titulo_receber_rateio_cc TPC ' +
                       ...
                       qryAjustaRateioCC.ParamByName('PVALOR').AsCurrency :=(pValor);
...
pValorHist, pValorCC,  pValorPago, pValor :Currency;
alterarCC, alterarHistorico :Boolean;
begin
    qtdHistorico:=0;
    tempo :=0;
    ...
    framePB1.lblProgressBar.Caption := 'Existem ' + IntToStr(qryObterTodosTPPeriodo.RecordCount) + ' a serem ajustados. ';
      ...
      try
        ...
        while not (qryObterTodosTPPeriodo.Eof) do begin
          ...
          pValorPago   := qryObterTodosTPPeriodo.FieldByName('VALOR_PAGO').AsCurrency;
          idHistorico := qryObterTodosTPPeriodo.FieldByName('ID_HISTORICO').AsString;

          lbl4.Caption := 'Registro atual Nº ' + IntToStr(qryObterTodosTPPeriodo.RecNo) + #13 +
                          'DOC ' +  qryObterTodosTPPeriodo.FieldByName('NUMERO_DOCUMENTO').AsString;
                    ...
                    if (qtdCC = 1) then begin
                     ...
                     if (alterarCC)  then begin
                      ...
                      sqlAjusta := ' UPDATE titulo_pagar_rateio_cc TPC ' +
                       ...
                       qryAjustaRateioCC.ParamByName('PVALOR').AsCurrency :=(pValorPago);
                       qryAjustaRateioCC.ExecSQL;
                       qryAjustaRateioCC.Connection.CommitRetaining;
                     end;
...
begin
   ...
   //ajusta data de emissao que esteja menor que 2 anos.
   if( AjustarDataEmissaoTP(ObterTodosTitulosPagar(TOrgAtual.getId,'TODOS'))) then begin
...
begin
  ...
  if AjustaDataPagamentoTitulo('TITULO_PAGAR', ObterTodosTitulosPagar(idOrganizacao, 'QUITADO')) then begin
...
begin
   ...
   lbl4.Caption := 'Ajustando registros ';
       ...
       qryObterTPPVazio.Open;
        ...
        qryObterTRPVazio.Close;
        qryObterTRPVazio.Connection := dmConexao.conn;
        qryObterTRPVazio.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
        qryObterTRPVazio.Open;

        if AjustaRegistroProvisao('TITULO_PAGAR', qryObterTPPVazio) then
        begin

          if AjustaRegistroProvisao('TITULO_RECEBER', qryObterTRPVazio) then
          begin

            ShowMessage('Processo concluído para titulos a pagar/receber');
          end;
...
procedure TfrmManutencao.btnBaixaPesquisaClick(Sender: TObject);
var
pIDTPB, pDoc :string;
...
begin
  ...
  pDoc := edtBaixaNumDoc.Text;
  DBGridBXFP.DataSource.DataSet.Close;
  DBGridBXWWW.DataSource.DataSet.Close;
  DBGridBXHistorico.DataSource.DataSet.Close;
  DBGridBXCentroCusto.DataSource.DataSet.Close;
  DBGridBXCAIXA.DataSource.DataSet.Close;
  DBGridBXCH.DataSource.DataSet.Close;
  DBGridBaixasTP.DataSource.DataSet.Close;
  ...
  if not uutil.Empty(pDoc) then begin
     tituloPagar := TTituloPagarModel.Create;
     tituloPagar :=  ObterTodosTitulosPagarDOC(idOrganizacao, pDoc);

     if not uutil.Empty(tituloPagar.FID) then begin

          edtFornecedor.Text  := tituloPagar.FCedente.Fnome;
          edtDescricao.Text   := tituloPagar.FHistorico.FDescricao + ' ' +  tituloPagar.Fdescricao;
          edtStatus.Text      := tituloPagar.FTipoStatus.Fdescricao;
        ...
        qryObterRateioHistorico.ParamByName('PIDORGANIZACAO').AsString := tituloPagar.FIDorganizacao;
        qryObterRateioHistorico.ParamByName('PIDTP').AsString := tituloPagar.FID;
        qryObterRateioHistorico.Open;
        //centro de custos
        qryObterRateioCC.Close;
        qryObterRateioCC.Connection := dmConexao.conn;
        qryObterRateioCC.ParamByName('PIDORGANIZACAO').AsString := tituloPagar.FIDorganizacao;
        qryObterRateioCC.ParamByName('PIDTP').AsString := tituloPagar.FID;
        qryObterRateioCC.Open;


        //baixas do titulo
        qryObterBaixaDOCTP.Close;
        qryObterBaixaDOCTP.Connection := dmConexao.conn;
        qryObterBaixaDOCTP.ParamByName('PIDORGANIZACAO').AsString := tituloPagar.FIDorganizacao;
        qryObterBaixaDOCTP.ParamByName('PID').AsString := tituloPagar.FID;
        qryObterBaixaDOCTP.Open;


       {
        DBGridBaixasTP.DataSource.DataSet.Close;
        DBGridBaixasTP.DataSource.DataSet := qryObterBaixaDOCTP;
        DBGridBaixasTP.DataSource.DataSet.Refresh; }


     end else begin PempecMsg.MsgError('Documento não encontrado.'); end;
...
procedure TfrmManutencao.btnDeleteBaixaDocTPClick(Sender: TObject);
var
 sqlDelTD, sqlDelCBD, pIDTPB, pIDTP :string;
 sqlDelTPB, sqlDelTPBWW, sqlDelTPFP :string;

qryDeletaBaixa :TFDQuery;


begin
  qryDeletaBaixa := TFDQuery.Create(Self);

  sqlDelTD    := ' DELETE FROM tesouraria_debito TD WHERE (TD.ID_ORGANIZACAO = :PIDORGANIZACAO) AND (TD.ID_TITULO_PAGAR_BAIXA = :PIDTPB) ';
  sqlDelCBD   := ' DELETE FROM conta_bancaria_debito cbd WHERE (cbd.ID_ORGANIZACAO  = :PIDORGANIZACAO)  AND (cbd.ID_TITULO_PAGAR = :PIDTP)';
  sqlDelTPBWW := ' DELETE FROM titulo_pagar_baixa_internet TPBW WHERE (TPBW.ID_ORGANIZACAO = :PIDORGANIZACAO)  AND (TPBW.ID_TITULO_PAGAR_BAIXA = :PIDTPB) ' ;
  sqlDelTPFP  := ' DELETE FROM titulo_pagar_baixa_fp TPBFP WHERE (TPBFP.ID_ORGANIZACAO = :PIDORGANIZACAO) AND (TPBFP.ID_TITULO_PAGAR_BAIXA = :PIDTPB) ';
  sqlDelTPB   := ' DELETE FROM titulo_pagar_baixa TPB WHERE (TPB.ID_ORGANIZACAO = :PIDORGANIZACAO)  AND (TPB.ID_TITULO_PAGAR_BAIXA = :PIDTPB) ';


   pIDTP  := qryObterBaixaDOCTP.FieldByName('pIDTP').AsString;   //usado em  sqlDelCBD
   pIDTPB :=  qryObterBaixaDOCTP.FieldByName('pIDTPB').AsString;

   // remove a baixa com base nesse ID
   try
    //remove o lancamento  conta bancaria DB
     qryDeletaBaixa.Close;
     qryDeletaBaixa.Connection := dmConexao.conn;
     qryDeletaBaixa.SQL.Clear;
     qryDeletaBaixa.SQL.Add(sqlDelCBD);
     qryDeletaBaixa.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
     qryDeletaBaixa.ParamByName('PIDTP').AsString := pIDTP;
     qryDeletaBaixa.ExecSQL;


   //remove o lancamento da tesouraria debito

     qryDeletaBaixa.Close;
     qryDeletaBaixa.Connection := dmConexao.conn;
     qryDeletaBaixa.SQL.Clear;
     qryDeletaBaixa.SQL.Add(sqlDelTD);
     qryDeletaBaixa.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
     qryDeletaBaixa.ParamByName('PIDTPB').AsString := pIDTPB;
     qryDeletaBaixa.ExecSQL;


      //remove o lancamento da TPBInternet

     qryDeletaBaixa.Close;
     qryDeletaBaixa.Connection := dmConexao.conn;
     qryDeletaBaixa.SQL.Clear;
     qryDeletaBaixa.SQL.Add(sqlDelTPBWW);
     qryDeletaBaixa.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
     qryDeletaBaixa.ParamByName('PIDTPB').AsString := pIDTPB;
     qryDeletaBaixa.ExecSQL;

     //remove o lancamento da TPBFP

     qryDeletaBaixa.Close;
     qryDeletaBaixa.Connection := dmConexao.conn;
     qryDeletaBaixa.SQL.Clear;
     qryDeletaBaixa.SQL.Add(sqlDelTPFP);
     qryDeletaBaixa.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
     qryDeletaBaixa.ParamByName('PIDTPB').AsString := pIDTPB;
     qryDeletaBaixa.ExecSQL;


      //remove o lancamento da TPB

     qryDeletaBaixa.Close;
     qryDeletaBaixa.Connection := dmConexao.conn;
     qryDeletaBaixa.SQL.Clear;
     qryDeletaBaixa.SQL.Add(sqlDelTPB);
     qryDeletaBaixa.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
     qryDeletaBaixa.ParamByName('PIDTPB').AsString := pIDTPB;
     qryDeletaBaixa.ExecSQL;
    ...
    raise Exception.Create('Erro ao tentar deletar baixa por numero documento.');

   end;


   PempecMsg.MsgInformation('Processo concluído com sucesso!!!');
   btnBaixaPesquisa.Click;
...
procedure TfrmManutencao.dsBaixasTPDataChange(Sender: TObject; Field: TField);
var pIDTPB, pDoc :string;
begin

      if qryObterBaixaDOCTP.RecordCount > 0 then
      begin
         btnDeleteBaixaDocTP.Enabled := True;
         pIDTPB :=  qryObterBaixaDOCTP.FieldByName('pIDTPB').AsString;

        qryBaixaDOCTPFP.Close;
        qryBaixaDOCTPFP.Connection := dmConexao.conn;
        qryBaixaDOCTPFP.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
        qryBaixaDOCTPFP.ParamByName('PIDTPB').AsString := pIDTPB;
        qryBaixaDOCTPFP.Open;

        //baixas pelo caixa

        qryBaixaDOCTPCAIXA.Close;
        qryBaixaDOCTPCAIXA.Connection := dmConexao.conn;
        qryBaixaDOCTPCAIXA.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
        qryBaixaDOCTPCAIXA.ParamByName('PIDTPB').AsString := pIDTPB;
        qryBaixaDOCTPCAIXA.Open;

        //BAIXAS POR WWW
        qryBaixaDOCTPWWW.Close;
        qryBaixaDOCTPWWW.Connection := dmConexao.conn;
        qryBaixaDOCTPWWW.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
        qryBaixaDOCTPWWW.ParamByName('PIDTPB').AsString := pIDTPB;
        qryBaixaDOCTPWWW.Open;

        //BAIXAS POR CHEQUE
        qryBaixaDOCTPCH.Close;
        qryBaixaDOCTPCH.Connection := dmConexao.conn;
        qryBaixaDOCTPCH.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
        qryBaixaDOCTPCH.ParamByName('PIDTPB').AsString := pIDTPB;
        qryBaixaDOCTPCH.Open;

        if qryBaixaDOCTPCH.RecordCount >0 then begin
          btnDeleteBaixaDocTP.Enabled := False;
        end;
...
pIDTP :string;
begin
    edtTotalRateioCC.Value := 0;
    edtDIFRateioCC.Value := 0;
    ...
    obterRateioCC(uutil.TOrgAtual.getId,pIDTP,'TITULO_PAGAR');
     ...
     dbgCC.Refresh;
     edtTotalRateioCC.Value := obterSomaPorCampo('VALOR' ,qryObterRateioCC);
     edtDIFRateioCC.Value := qryObterTodosTPB.FieldByName('VALOR_PAGO').AsCurrency - edtTotalRateioCC.Value ;
        ...
        try
          ...
          begin
            ...
            begin
              ...
              tituloRC := TTituloReceberModel.Create;
              tituloRC.FIDorganizacao := idOrganizacao;
              tituloRC.FID := pID;

              tituloRC := TTituloReceberDAO.obterPorID(tituloRC);

              if not uutil.Empty(tituloRC.FID) then
              begin
                TTituloReceberDAO.Update(tituloRC);
              end;
...
function TfrmManutencao.ObterTodosTitulosPagar(pIdOrganizacao, pIdStatus :string): TFDQuery;
...
begin
  ...
  titulo.FIDTipoStatus :=  pIdStatus; //  'QUITADO';
  qryTodosTP.Close;
...
function TfrmManutencao.ObterTodosTitulosPagarDOC(pIdOrganizacao,  pDOC: string): TTituloPagarModel;
  var
  titulo : TTituloPagarModel;
begin
    ...
    try
      titulo := TTituloPagarModel.Create;
      titulo.FIDorganizacao := pIdOrganizacao;
      titulo.FnumeroDocumento := pDOC;


      titulo := TTituloPagarDAO.obterPorNumeroDocumento(titulo)

    except
    raise Exception.Create('Erro ao obter filhos TP por DOC');
 ...
 dmConexao.conectarBanco;
     { substituido em 12/06/20 ranan
      ...
      if pTable.Equals('TITULO_RECEBER') then begin
        ...
        cmdSql := ' SELECT TPC.id_titulo_RECEBER_rateio_cc, '+
                  ' TPC.id_titulo_receber, TPC.valor '+
...
function TfrmManutencao.insereCentroRateioCentroCusto(pIdOrganizacao, pIdTitulo, pIdCentroCusto,  pTable: string ; pValor :Currency): Boolean;
var
