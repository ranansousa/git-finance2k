...
uses
  ...
  uFrameHistorico, cxGraphics, cxControls, cxLookAndFeels,
...
procedure TfrmCTPHistorico.btnFecharClick(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    begin
      if obterTitulosPorFornecedor(idOrganizacao,pIdHistorico,  idCedente,
...
procedure TfrmCTPHistorico.frameHistorico1cbbcomboChange(Sender: TObject);
...
begin
  ...
  if frameHistorico1.cbbcombo.ItemIndex >0  then begin
       pIdHistorico := FsListaIdHistorico[frameHistorico1.cbbcombo.ItemIndex];
       conta := frameHistorico1.getContaContabil(idOrganizacao, pIdHistorico) ;

       if not uutil.Empty(conta.FConta) then begin
          edtCCHistorico.Text := conta.FConta;
       end else begin
        edtCCHistorico.Text := 'NC';
       end;
