...
type
  ...
  public
   ...
   constructor Create(AOwner: TComponent; nDoc :String);
...
//pathRelatorio := uutil.TPathRelatorio.getContasPagarEspelho;
 ...
 if (dmEspelhoTP.obterPorNumeroDocumento(idOrganizacao, pNumeroDoc)) then begin
  ...
  function TformEspeloTP.obterIndiceLista(pId: string;  lista: TStringList): Integer;
var
  I: Integer;
begin
 Result :=0;
...
constructor TformEspeloTP.Create(AOwner: TComponent; nDoc: String);
var
...
begin
  ...
  limpaPanel;
    ...
    if not uutil.Empty(tituloEntrada.FID) then begin
         ...
         pNumeroDoc := nDoc;
         frameTP1.cbbTP.ItemIndex := obterIndiceLista(tituloEntrada.FID, FsListaIdTP);
        // frameTP1.cbbTP.Enabled := False;
