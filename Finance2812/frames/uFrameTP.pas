...
type
  ...
  public
    { Public declarations }
  function obterTodosInconsistentes(pIdValue: string; var combo: TComboBox; var listaID: TStringList): boolean;
...
function TframeTP.obterTodosPorPeriodo(pIdValue: string; pDataInicio,   pDataFinal: TDateTime; var combo: TComboBox; var listaID: TStringList): boolean;
var
cmdSql :string;
pSigla,pTable, pCampoSelecao, pCampoID :string;
begin
...
pCampoID := 'ID_'+ pTable;
 ...
 cmdSql := 'SELECT ' + pCampoSelecao +',' + pCampoID  +
           ' FROM ' + pTable + ' '+ pSigla +
           ' WHERE ( ' + pSigla + '.ID_ORGANIZACAO = :PIDORGANIZACAO) ' +
           ' AND  ( ( ' + pSigla + '.DATA_VENCIMENTO  BETWEEN :PDTINICIO AND :PDTFIM ) ' +
           ' OR   ( ' + pSigla + '.DATA_PAGAMENTO  BETWEEN :PDTINICIO AND :PDTFIM ) )' +
           ' AND   ( ' + pSigla + '.ID_TIPO_STATUS  <> ''EXCLUIDO'' ) ' +
           ' ORDER BY '+ pSigla + '.DATA_VENCIMENTO desc , TP.NUMERO_DOCUMENTO ' ;
...
function TframeTP.obterTodosInconsistentes(pIdValue: string; var combo: TComboBox;  var listaID: TStringList): boolean;
var
cmdSql :string;
pSigla,pTable, pCampoSelecao, pCampoID :string;
sucesso : Boolean;
