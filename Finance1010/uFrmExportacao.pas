...
type
  ...
  private
    ...
    idEmpresaSistemaContabil :string;
...
begin
  ...
  codEmpresa := StrToInt(TOrgAtual.getIDSistemaContabil);
  qtdReg := 0;
    ...
    begin
      ...
      try
        ...
        try
         ...
         lista := dmExportaFinance.obterCaixaBancoPorPeriodo(idOrganizacao, ano, pDataInicial, pDataFinal, codEmpresa, lote, loMostraErros);
...
begin
  ...
  // dmOrganizacao.carregarDadosEmpresa(idOrganizacao); ver
   ...
   // Essa parte é só para impressão e liberação dos botões de exportação
     ...
     if (cbxMesLote.ItemIndex >0 ) then begin defineLote end;
...
begin
  ...
  idEmpresaSistemaContabil := TOrgAtual.getIDSistemaContabil;
  frmPeriodo1.inicializa(Now, Now);
  statusExportar(0);
  limpaPainelExportacao;
...
begin
  ...
  begin
    ...
    codEmpresa := StrToInt(TOrgAtual.getIDSistemaContabil);
    if codEmpresa = 0 then begin
       try
       codEmpresa := dmMegaContabil.obterIDEmpresa(TOrgAtual.getCNPJ);
       TOrgAtual.setIDSistemaContabil(IntToStr(codEmpresa));
       except on E: Exception do
        ShowMessage(' '+ e.Message);
       end;
...
begin
  ...
  begin
    ...
    if (dmExportaFinance.gravarLoteContabil('FNCEXP@' + ano + '@' + IntToStr(lote) + '@' + IntToStr(linha), idOrganizacao,
        IntToStr(lote),  FormatDateTime('DD/MM/YYYY', frmPeriodo1.getDataInicial), FormatDateTime('DD/MM/YYYY', frmPeriodo1.getDataFinal),
