object dmExportaFinance: TdmExportaFinance
  ...
  object qryObterPlanoHistoricoMega: TFDQuery
    ...
    SQL.Strings = (
      'select '
      '    cplanohistorico.cdhist,'
      '    cplanohistorico.historico,'
      '    cplanohistorico.id_cplanohistorico,'
      '    cplanohistorico.tipo_fluxo_caixa,'
      '    cplanohistorico.descricao_fluxo_caixa'
      'from cplanohistorico'
      'order by cplanohistorico.cdhist')
    Left = 1012
    Top = 472
  end
  object qryObterHistoricoPadraoMega: TFDQuery
    Connection = dmConexao.ConnMega
    FormatOptions.AssignedValues = [fvDefaultParamDataType, fvFmtDisplayDate, fvFmtDisplayNumeric, fvFmtEditNumeric]
    FormatOptions.DefaultParamDataType = ftString
    FormatOptions.FmtDisplayDate = 'DD/MM/YYYY'
    ...
    SQL.Strings = (
      ...
      ''
      'from cplanohistorico'
      'where cplanohistorico.cdhist = :PCODIGO')
    Left = 1012
    Top = 528
    ParamData = <
      item
  ...
  object qryListaHistoricos: TFDQuery
    Connection = dmConexao.conn
    SQL.Strings = (
      
        'SELECT H.id_historico, h.id_organizacao, h.descricao, h.tipo, h.' +
        'codigo, h.id_conta_contabil'
      ''
      'FROM HISTORICO H'
      ''
