...
uses
  ...
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet,uTPBaixaDAO,uTPBaixaModel,
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              tituloPagar.FregistroLoteDeletado := lotePagto.Flote;
...
dmConexao.conectarBanco;
 ...
 sqlCmd := ' SELECT LP.id_lote_pagamento, LP.LOTE as LOTE, LP.id_organizacao, LP.DATA_PAGAMENTO, LP.VALOR, LP.status, LP.QTD_TIT_PAG as QTD, '+
      ...
      ' ORDER BY LP.DATA_PAGAMENTO DESC, LP.VALOR DESC ';
 ...
 begin
   sqlCmd := ' SELECT LP.id_lote_pagamento, LP.LOTE as LOTE, '+
      'LP.id_organizacao, LP.DATA_PAGAMENTO, LP.VALOR, '+
      ' LP.status, LP.QTD_TIT_PAG as QTD, '+
      ' FP.id_forma_pagamento, FP.descricao AS FP, '+
      ' CBC.id_conta_bancaria_cheque, CBC.numero_cheque AS CHEQUE, CBC.id_conta_bancaria, '+
      ' CB.id_conta_bancaria, CB.conta '+
      ' FROM lote_pagamento LP '+
  ...
  try
    ...
    TNumericField(qryPreencheDBGrid.FieldByName('VALOR')).DisplayFormat := ',0.00;-,0.00';


  except
    on e: Exception do
      PempecMsg.MsgInformation(e.Message + sLineBreak + ' Não foi possível acessar os lotes. ');
