 ...
 //se falhar acima. Pega o CNPJ  direto no database FInance
  ...
  begin
    ...
    begin
      ...
      PempecMsg.MsgInformation('Lote já existe no Sistema Contábil. ');
      fechamento := false;
  ...
  begin
    PempecMsg.MsgInformation('Será preciso selecionar uma Organização antes de prosseguir.');
    fechamento := false;
    dxBtnImprimir.Enabled := false;
    Limpatela(Self);
    limpaPainelExportacao;
  ...
  begin
    ...
    begin
      ...
      Limpatela(Self);
      PempecMsg.MsgInformation('Tem fechamento registrado. Não será possível continuar. Verifique o periodo informado. ');
    end;
    if not (idEmpresaSistemaContabil = (-1)) then
    begin
    ...
    begin
      ...
      try
        ...
        //Gravando CLOTES para CBT
          ...
          begin
            ...
            begin
              ...
              limpaPainelExportacao;
              PempecMsg.MsgInformation('Não foi possível gravar o lote no sistema contábil. ' + pTipo);
            end;
          ...
          begin
            limpaPainelExportacao;
            PempecMsg.MsgInformation('Não foram encontrados lançamentos ' + pTipo + ' para exportar.');
          end;
        ...
        on e: Exception do
          PempecMsg.MsgInformation(e.Message + sLineBreak + 'Erro ao tentar gravar o lote no Sistema Contábil! ' + pTipo);
      end;
    ...
    begin
      ...
      try
        ...
        //Gravando CLOTES para CBT
          ...
          begin
            limpaPainelExportacao;
            PempecMsg.MsgInformation('Não foram encontrados lançamentos para exportar. ' + pTipo);
          end;
        ...
        //Gravando CLOTES para CBT
          ...
          begin
            ...
            begin
              ...
              imgGravaLote.Visible := False;
              limpaPainelExportacao;
              PempecMsg.MsgInformation('Não foi possível gravar o lote no sistema contábil.');
            end;
      ...
      try
       ...
       //   vDebito := dmExportaFinance.obterValorDebitoTPB(idOrganizacao, pTable, 'VALOR_NOMINAL', 'DATA_PAGAMENTO', pDataInicial, pDataFinal, 1);
        ...
        //Gravando CLOTES para CBT
          ...
          begin
            limpaPainelExportacao;
            PempecMsg.MsgInformation('Não foram encontrados lançamentos ' + pTipo + ' para exportar ou ocorrem problemas durante o processamento.');
          end;
...
begin
 //pesquisa cheques que tenham sido emitidos no periodo a ser exportado e impede a exportação
...
var
 I, aux: Integer;
  ...
  qdtTP: Integer; //conta qtos titulos estao em aberto no periodo de exportacao. bloqueia a exportação
  vDebito: Currency;
...
begin
 { vlrCBT :=0; vlrTP:=0; vlrTR:=0; vlrTPROV:=0; vlrTRPROV:=0; vlrTBT:=0; vlrTCB:=0;
  ...
  if qdtTP > 0 then begin
      ...
      loMostraErros.Add('               Existe(m) título(s) em aberto no período informado.', False, True);
     ...
     loMostraErros.Add(' ', False, True);
      loMostraErros.Add('               Efetue a baixa do(s) título(s) ou altere a data de VENCIMENTO. ', True, True);
  ...
  // retirado em 10/06. Começando usar o DAO vDebito := dmExportaFinance.obterValorDebitoGenericTOB(idOrganizacao, 'CONTA_BANCARIA_TRANSFERENCIA', 'VALOR', tipoLancamento, pDataInicial, pDataFinal);
  ...
  begin
   ...
   // retirado em 10/06. Começando usar o DAO dmRelExportacaoMega.obterCBT(idOrganizacao, pDataInicial, pDataFinal);
 ...
 vlrTRPROV:=vDebito;
  ...
  begin
    PempecMsg.MsgInformation('Não existem dados para o período informado...');
  end;
...
begin
  ...
  begin
    ...
    lblStatusSistemaContabil.Caption := ' VERIFIQUE OS LOTES IMPORTADOS PARA O SISTEMA CONTÁBIL...';
    PempecMsg.MsgWarning('Não será possível prosseguir com a exportação. ' + #13 + 'VERIFIQUE OS LOTES IMPORTADOS PARA O SISTEMA CONTÁBIL.');
...
begin
 ...
 // pDataFinal := StrToDate(FormatDateTime('dd"/"mm"/"yyyy', dxDataFinal.Date));
  ...
  begin
    ...
    loMostraErros.Add(' Existem cheques emitidos no período informado.' , False, True);
    loMostraErros.Add(' A exportação de títulos quitados não será permitida até que os cheques sejam compensados.', False, True);
    loMostraErros.Add(' A falta da compensação do cheque provocará diferença no saldo da conta bancária. ' , False, True);
  ...
  if not dmExportaFinance.validarCodigosHistoricos(uutil.TOrgAtual.getId, loMostraErros ) then begin
...
begin
  ...
  begin
    ...
    tamLista := TListaLancamentoCredito(lista).Count;   //usado só na barra de progresso
    ...
    begin
     ...
     //ajustar o lote contabil aqui pegando o valor do debito e do credito.
      // só irá analisar o valor que for maior que 0.
...
begin
  chequesEmtidos :=0;
  aux := True;
   msgStatus := 'BASE DE DADOS DO SISTEMA CONTÁBIL CONECTADA. ';
  ...
  begin
    aux := False;
    msgStatus := ' SISTEMA CONTÁBIL NÃO ENCONTRADO';
    msgStatusBar(3,msgStatus);
    Application.ProcessMessages;
  end;
  ...
  if verificaErroLoteMega(idEmpresaSistemaContabil) then begin
     ...
     msgStatus := ' VERIFIQUE OS LOTES IMPORTADOS PARA O SISTEMA CONTÁBIL...';
  ...
  { mudei em 9/10. fica perto de verificacao de titulo aberto
    ...
    begin
      ...
      begin
        ...
        loMostraErros.Add('Existem cheques emtitidos no período informado. A exportação de títulos quitados não será permitida até que os cheques sejam compensados.', False, True);

//      PempecMsg.MsgInformation('Existem cheques emtitidos no período informado. A exportação de títulos quitados não será permitida até que os cheques sejam compensados.');
