...
//      dbgrdMain.DataSource.DataSet.GotoBookmark(pointer(dbgrdMain.SelectedRows.Items[dbgrdMain.DataSource.DataSet.RecNo])); //mandando imprimir apenas o registro 0
      dbgrdMain.DataSource.DataSet.GotoBookmark(dbgrdMain.SelectedRows.Items[dbgrdMain.DataSource.DataSet.RecNo]);


            for I := 0 to dbgrdMain.SelectedRows.Count - 1 do
            begin
              //dbgrdMain.DataSource.DataSet.GotoBookmark(pointer(dbgrdMain.SelectedRows.Items[I]));

              dbgrdMain.DataSource.DataSet.GotoBookmark(dbgrdMain.SelectedRows.Items[I]);

            end;

        dbgrdMain.DataSource.DataSet.RecNo;
...
//        GotoBookmark(pointer(dbgrdMain.SelectedRows.Items[I]));
  ...
  if aux > 0 then
  begin
    preencheStatus(1, 'Serão impressos ' + IntToStr(aux) + ' espelhos. ');
...
begin
  aux := 0;
  dmEspelhoTP.fdmTPPrinter.Close;
  dmEspelhoTP.qryTPPROVCR.Close;
  dmEspelhoTP.qryTPPROVDB.Close;
  dmEspelhoTP.qryTPQuitados.Close;
  dmEspelhoTP.qryTPBDeducao.Close;
  dmEspelhoTP.qryBaixaTPCaixa.Close;
  dmEspelhoTP.qryTPBHistorico.Close;
  dmEspelhoTP.qryTPBAcrescimos.Close;
  dmEspelhoTP.qryBaixaTPCheque.Close;
  dmEspelhoTP.qryObterTPBBanco.Close;
  dmEspelhoTP.qryLotePagamento.Close;
  dmEspelhoTP.qryRateioCentroCustos.Close;
  dmEspelhoTP.qryObterTPBaixaPorTitulo.Close;
  dmEspelhoTP.qryObterPorNumeroDocumento.Close;
 ...
 try
  ...
  if not uutil.Empty(pIDTP) then begin
    ...
    sqlCmd :=' SELECT TP.ID_TITULO_PAGAR as ID, '+
       ...
       'TP.NUMERO_DOCUMENTO, '+
  'case '+
       ' when (tp.id_tipo_status in (''QUITADO'', ''PARCIAL'')) then TPB.VALOR_PAGO '+
       ' when (tp.id_tipo_status in (''ABERTO'')) then  TP.VALOR_NOMINAL '+
  ...
  begin
    ...
    begin
     ...
     // Variables['strTipoStatus'] := QuotedStr(pidStatus);
      Variables['strPeriodo'] :=QuotedStr( ' De  ' + DateToStr(dxBarDTINI.Date) + '  até  ' + DateToStr(dxBarDTFIN.Date));
