...
uses
  Winapi.Windows, uMD5, Winapi.Messages, System.SysUtils, System.UITypes, System.Variants, System.Classes, Vcl.Graphics, uUtil, uVarGlobais, Vcl.Controls,
  Vcl.Forms, Vcl.Dialogs, FireDAC.Phys.ADSWrapper, FireDAC.UI.Intf, FireDAC.VCLUI.Error, FireDAC.VCLUI.Wait, FireDAC.Phys, FireDAC.stan.Def, Registry,
   FireDAC.Phys.IBBase, FireDAC.Phys.FB, FireDAC.Comp.UI, FireDAC.stan.Intf, uFrmRegistraBaseDados, FireDAC.Phys.ADS, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.Buttons,
    FireDAC.Phys.IBWrapper, Vcl.Samples.Gauges, Vcl.ComCtrls,   Vcl.Samples.Spin, wininet, Winsock, IdSSL, IdSSLOpenSSL, IdMessage, Organizacao, IdAttachmentFile,
     IdTCPClient, IdExplicitTLSClientServerBase, IdMessageClient, IdSMTPBase, uLogin, IdTCPConnection, IdSMTP, jpeg, IdBaseComponent, IdComponent, IdHTTP, FireDAC.stan.Option,
      FireDAC.stan.Error, FireDAC.Phys.Intf, FireDAC.stan.Pool, FireDAC.stan.Async, FireDAC.stan.Param, FireDAC.DatS, FireDAC.DApt.Intf, FireDAC.DApt, FireDAC.Comp.Client, Data.DB,
       FireDAC.Comp.DataSet, Vcl.Mask, VCLUnZip, VCLZip, FireDAC.Phys.FBDef, IdAntiFreezeBase, Vcl.IdAntiFreeze, frxClass, udmConexao, uDMOrganizacao, udmMegaContabil, uDMUsuarioConsulta,
       ACBrBase, uDmExportaFinance, uDMContasPagar, Vcl.Imaging.pngimage, Vcl.Grids, Vcl.DBGrids, frxDBSet, uFrmManutencao, Vcl.Menus, uFrmBackup, uFrmRegistro, uFrmServidorEmail,
        System.ImageList, Vcl.ImgList, uFrmImportacao, uFrmRelatorios, uFrmExportacao, uFrmSincronizaMega, uFrmTeste, uFrmUpdate, udmCombos, RxDBCtrl, frxCrypt,
         UMostraErros,uFrmAlteraTituloReceber, uFrameBDTables, udmManutencao, uFrmExtratoBancario, uFrameOrganizacoes, uFrameEstado, uFrmDeletaLoteContabil, uFrmCompensaCheque, uFrmMultiplosCheques,
  uFrameGeneric, uFrameCidade, uFrameEndereco, uFrameBairro, uDMServerMail,uFrmAlteraOrganizacao,uFrmSelecionaOrganizacao,
  FireDAC.Comp.ScriptCommands, FireDAC.Stan.Util, FireDAC.Comp.Script,uFrmSaldoContas,UfrmPosicaoFinanceira, uFramePeriodo, uDMParametros, uFrameHistorico, sSkinManager, cxGraphics, uFrmDemonstrativoRD, uFrmRelatoriosPagamentos,uFrmRelatorioPagamentosHistorico,
  cxControls, cxLookAndFeels, cxLookAndFeelPainters, dxRibbonSkins, dxSkinsCore, uFrmExtratoTesouraria, uFrmTelaImpressaoLotePagamento,
  dxSkinsDefaultPainters, dxRibbonCustomizationForm, dxStatusBar, uFrmHistorico, uFrmManterHistorico,uFrmManterContaBancaria,
  dxRibbonStatusBar, cxClasses, dxRibbon, Vcl.PlatformDefaultStyleActnCtrls,uFrmLoteDeposito,uFrmManterTipoStatus,
  System.Actions, Vcl.ActnList, Vcl.ActnMan, dxBar, cxButtons, ACBrCalculadora, uFrmTransferenciasEntreContas, System.DateUtils,
  dxBarApplicationMenu, cxCalendar, sMaskEdit, sCustomComboEdit, sCurrEdit,  uFrmTransfereBancoCaixa,uFrmTransfereCaixaBanco,
  cxCalc, dxDateTimeWheelPicker, cxBarEditItem, cxContainer, cxEdit, uFrmAjustaSkin, uFrmDepositoCheque,uFrmGerarCheques,uFrmManterFuncionario,
  uFrmManterTipoCedente, uFrmManterTipoSacado, uFrmManterTipoNotaFiscal, uFrmManterTipoCobranca, uFrmManterTOB,uFrmManterCartaoCredito,
  dxWheelPicker, dxNumericWheelPicker, cxDropDownEdit, ENumEd, uFrmEspelhoTP,uFrmImobilizado,  uFrmManterCedente,uFrmManterSacado,
  uFrameCheque,uFrmManterTipoDeducao,uFrmManterTipoAcrescimo, uFrmReciboTP, uFrmReciboTR, cxTextEdit,uFrmTelaPagamentoTP ,
  uFrmManterTituloReceber,uFrmManterTituloPagar,  uUsuarioModel, uUsuarioDAO,uFrmTelaPagamentoTR,uFrmTelaPagamentoLoteTP,
  EMsgDlg, cxButtonEdit, MDDAO, uOrganizacaoDAO, uOrganizacaoModel,uFrmManterUsuario, uFrmEstornaCheque,
  cxCheckBox, dxBarExtItems,uFrmInfoUpdate  ;
...
begin
  ...
  begin
    if not PempecMsg.MsgConfirmation('Você tem certeza que gostaria de salvar seus dados de login neste computador? ' + #13 + ' Caso aceite, sua decisão poderá ser revertida em Parâmetros->Usuario .') = 6 then
    begin
...
begin
  ...
  begin
    ...
    begin
      PempecMsg.MsgInformation('Usuário não localizado... Selecione um usuário...');
    end;
...
if( cbxUsuario.ItemIndex < 0) then begin
   ...
   PempecMsg.MsgInformation('Seleção inválida!!!!');
end;
end;
...
begin
  msgPadrao := 'Não foi possível obter as informações. ' + #13 + ' Por favor, tente novamente mais tarde. ' ;
  ...
  begin
    ...
    begin
      msgPadrao := 'Existem novas atualizações. ' + #13 + ' Acesse o site www.pempec.com.br para baixar. ';
      try
...
begin
  ...
  try
        ...
        if (dmOrganizacao.carregarDadosOrganizacao(pIdOrganizacao)) then begin
          ...
          if not uUtil.Empty(organizacaoModel.FID) then begin uutil.TOrgAtual.setOrganizacao(organizacaoModel); end;
         ...
         try
          ...
          on e: Exception do
                PempecMsg.MsgInformation('Problemas ao tentar conectar com o sistema contábil...')
        ...
        begin
          PempecMsg.MsgInformation('Erro ao carregar os dados da organização...');
        end;
...
begin
 ...
 try
    ...
    raise(Exception).Create('Erro ao tentar obter dados do Sistema Contábil.');
 end;
...
// posicao 1 informa se é administrador
//posicao 2 informa o ip

dxStatusBar1.Panels[3].Text := 'Versão: ' + GetVersionInfo(Application.ExeName);
dxStatusBar1.Panels[4].Text := 'PC: ' + uutil.GetComputerNetName;
dxStatusBar1.Panels[5].Text := 'User : ' + uutil.GetUserFromWindows;
dxStatusBar1.Panels[6].Text := 'Internet : Sem conexão.  ' +  'Versão demonstrativa';
 ...
 //if 1=1 then   begin
    ...
    dxStatusBar1.Panels[6].Text := 'Internet : Sem conexão.  ' +  'Versão licenciada :' + uUtil.TOrgAtual.getFFLICENCA;
    if uutil.IsInternetConected then
    begin

      IPExterno:= TIPreal;

      dxStatusBar1.Panels[6].Text := 'Internet : ' + 'Conectado.  ' + 'IP : ' + IPExterno;
      try
        try
          versaoWeb := IdHTTP1.Get('http://pempec.com.br/validator/obterversao.php?sistema=finance');
...
begin
  ...
  begin
    ...
    begin
     ...
     // PempecMsg.MsgWarning('Existem novas atualizações. ' + #13 + ' Acesse o site www.pempec.com.br para baixar. ');
