...
begin
  ...
  begin
    permite := False;
    MessageDlg('O título foi pago em lote. Primeiro deve remover o lote pagamento : ' + edtLotePagamento.Text, mtInformation, [mbOK], 0);
  end;

  if not uUtil.Empty(edtLoteContabil.Text) then
  begin
    permite := False;
    MessageDlg('O título já foi exportado. Primeiro deve remover o lote contábil. ' + edtLoteContabil.Text, mtInformation, [mbOK], 0);
  end;
  ...
  begin
    ...
    begin
      ...
      MessageDlg('A baixa do título foi cancelada com sucesso. ' + edtLoteContabil.Text, mtInformation, [mbOK], 0);
      limparPanel;
...
begin
  ...
  begin
    try
      ...
      sucesso := False;
      raise Exception.Create('Erro ao inserir o TP');
...
begin
  ...
  { if not dmConexao.conn.InTransaction then
    ...
    try
      ...
      on e: Exception do
      ShowMessage(e.Message + sLineBreak + 'Contate o suporte. Informe erro: BTNSALVAR_TP ');
    end;
...
dmConexao.conectarBanco;
 ...
 if edtValorMainTP.Value = 0 then begin
    frameCentroCusto1.cbbcombo.ItemIndex :=0;
    MessageDlg('O valor do título não pode ser 0 ', mtInformation, [mbOK], 0);
  ...
  begin
    MessageDlg('Você precisa selecionar um centro de Custos. ', mtInformation, [mbOK], 0);
...
begin
 ...
 if frameTP1.cbbTP.ItemIndex >0 then begin
   ...
   tituloPagarModel := TTituloPagarDAO.obterPorID(tituloPagarModel);
    ...
    begin
      ShowMessage('Título não localizado... ');
      FreeAndNil(tituloPagarModel)
    end; 
    BtnGerarDOC.Caption := 'Consultar';
 end;
...
begin
 Result := True;
  ...
  begin
    ...
    frameCedente1.cbbcombo.Enabled := True;
    frameCedente1.cbbcombo.SetFocus;
    MessageDlg('Fornecedor sem conta contábil. Favor corrigir o problema. ', mtInformation, [mbOK], 0);
