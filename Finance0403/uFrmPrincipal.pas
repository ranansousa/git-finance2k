...
uses
  ...
  FireDAC.Comp.ScriptCommands, FireDAC.Stan.Util, FireDAC.Comp.Script,uFrmSaldoContas,UfrmPosicaoFinanceira,uOrganizacaoModel, uOrganizacaoDAO,
  ...
  uFrameCheque,uFrmManterTipoDeducao,uFrmManterTipoAcrescimo, uFrmReciboTP, uFrmReciboTR, cxTextEdit,uFrmTelaPagamentoTP ,   uFrmManterTituloPagar,  uUsuarioModel, uUsuarioDAO ;
   ...
   { usuario         := TUsuarioModel.create;
    ...
    begin
     ...
     if not uUtil.Empty(usuario.Flogin) then begin
      ...
      if not IsToday(TUsuarioDAO.obterUltimoAcesso(usuario)) then begin
...
on e: Exception do
      ShowMessage(e.Message + sLineBreak + 'Contate o suporte. Informe erro: MANTER FUNC ');
end;
end;
...
on e: Exception do
      ShowMessage(e.Message + sLineBreak + 'Contate o suporte. Informe erro: GERADOR CHEQUES') ;
end;
...
on e: Exception do
      ShowMessage(e.Message + sLineBreak + 'Contate o suporte. Informe erro: MANTER_HIST ') ;
end;
  ...
  try
        ...
        if (dmOrganizacao.carregarDadosOrganizacao(pIdOrganizacao)) then begin
         ...
         try
          ...
          on e: Exception do
                MessageDlg('Problemas ao tentar conectar com o sistema contábil...', mtWarning, [mbOK], 0);
