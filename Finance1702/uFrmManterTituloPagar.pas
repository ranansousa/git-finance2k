...
begin
   ...
   if modo = 1  then begin  //0 consulta - 1 ou mais titulo novo
     StatusBotoes(0);
...
begin
 if frameTP1.cbbTP.ItemIndex >0  then begin
   ...
   if not uutil.Empty(tituloPagarModel.FID) then begin
     ...
     ShowMessage('Titulo deletado com sucesso.');
...
begin
 ...
 //se for parcelado entra no outro formulario
...
begin
   //  pega os dados do form e seta no titulo
    ...
    tituloPagarModel.FIDCentroCusto         := FSlistaCentroCusto[frameCentroCusto1.cbbCentroCusto.ItemIndex];
...
begin
  ...
  begin
    ...
    //precisa ver a parte dos rateios
    //deletar os que já existem vinculados AO TP
      rateioHst := TTituloPagarHistoricoModel.Create;
      rateioHst.FIDorganizacao := pIDorganizacao;
      rateioHst.FIDTP := tituloPagarModel.FID;

      if TTituloPagarHistoricoDAO.DeletePorIDTP(rateioHst) then
      begin

     //insere os novos rateios
        for I := 0 to tituloPagarModel.listaHistorico.Count - 1 do
        begin
          TTituloPagarHistoricoDAO.Insert(tituloPagarModel.listaHistorico[I]);
      ...
      //rateio de centro de custos
       ...
       if TTituloPagarCentroCustoDAO.DeletePorIDTP(rateioCC) then begin

          for I := 0 to tituloPagarModel.listaCustos.Count -1  do begin


             TTituloPagarCentroCustoDAO.Insert(tituloPagarModel.listaCustos[I]);
    ...
    //nota fiscal. o form atualza o objeto
      if not uUtil.Empty(tituloPagarModel.FIDNotaFiscalEntrada) then
      begin
    //salvar nota fiscal
        if not tituloPagarModel.FNotaFiscalEntrada.Fnovo then
        begin //já existe

          TNotaFiscalEntradaDAO.Update(tituloPagarModel.FNotaFiscalEntrada);

        end
        else
        begin
          TNotaFiscalEntradaDAO.Insert(tituloPagarModel.FNotaFiscalEntrada);
        end;

      end;

      if not (tituloPagarModel.Fnovo) then  //já existe.. então atualiza
      begin

        TTituloPagarDAO.Update(tituloPagarModel);
      end
      else
      begin
        TTituloPagarDAO.Insert(tituloPagarModel);
      end;
      dmConexao.conn.CommitRetaining;
      dxBtnLimpar.Click;
    except
      dmConexao.conn.RollbackRetaining;
      raise Exception.Create('Erro ao tentar salvar os dados');
...
begin
 ...
 //remover os existentes
  if modo =0  then begin //update do tp

    //deleta os rateios que existem para colocar os novos

    for I := 0 to tituloPagarModel.listaCustos.Count-1 do begin
       try
       TTituloPagarCentroCustoDAO.Delete(tituloPagarModel.listaCustos[I]);
       except
       raise Exception.Create('Erro ao tentar limpar a lista de centro de custos em TPCC');
...
I,J :Integer;
begin
  { ISSO FAZ QDO VAI INSERIR ou ATUALIZAR O TP
 //se for TP de consulta.. precisa remover o que ja existe
     if modo =0  then begin //update do tp
    //deleta os rateios que existem para colocar os novos

    for I := 0 to tituloPagarModel.listaHistorico.Count-1 do begin
        try
        TTituloPagarHistoricoDAO.Delete(tituloPagarModel.listaHistorico[I]);
        except
        raise Exception.Create('Erro ao tentar limpar os historicos em TPH. ');
...
begin
 ...
 if frameTP1.cbbTP.ItemIndex >0 then begin
   ...
   tituloPagarModel := TTituloPagarDAO.obterPorID(tituloPagarModel);
        ...
        end else begin ShowMessage('Título não localizado... '); FreeAndNil(tituloPagarModel) end;
