...
type
  ...
  TInfoAplicacao = record
    ...
    bTkAtalhos: boolean;
    // bTkAtalhos não deixa as teclas F2...F6 serem executadas duas vezes
    bF9: boolean; // bF9 não deixa a calculadora ser executada duas vezes
    dblResultCalc: Double;
    // Variavél utilizada para retornar os valores da Calculadora F9
    sNomeComputador: String; // ShortString
    iScreenHeight: word; // Altura das telas filhas
    iScreenWidth: word; // Largura das telas filhas
    iBloqueio: ShortInt;
    // -1 = Não bloqueado, 0 = Bloqueio parcial,  1 = Bloqueio total, 2-Falta de atualização do programa
    dDtBloq: TDate; // Data do bloqueio
    sMsgBloq: String; // ShortString //Mensagem do bloqueio
    bAlertaBloq: boolean;
    // Caso seja verdadeiro, não blequeia o uso do sistema. Mostra apenas a mensagem
    bSysCorrompido: boolean; // Se o arquivo aplicacao.sys está corrompido.
    sExeTeste: String; // ShortString
    iDiasGeraSys: SmallInt;
    // Esta variavel é alimentada na rotina fGeraSys e contém a quantidade de dias que foi gerado o arquivo sys
    StBar: TStatusBar;
    { Guarda a referencia para a StatusBar do form principal. Pois, quando calcula a folha eu preciso passar a StatusBar por referência para o método AtualizaStatusBar para atualizar a data. }
    sFileVersion: String;
    { Guarda a versão do aplicativo. Pois, quando calcula a folha eu preciso passar para o método AtualizaStatusBar. }
    { As variáveis strListTabelasGerais e strListTabelasEspecificas
      devem ser criadas no create do Form principal da aplicação atráves de
      chamada ao método TabelasDaAplicacao. Os strLst com
      o sufixo Amigavel, contém os nomes amigáveis das tabelas.
      O StrLst com sufixo Gerais, guarda os nomes das tabelas que são utilizadas
      por todas as aplicações. O StrLst com sufixo especificas, guarda os nomes
      das tabelas específicas da aplicação/ }
    ...
    { Tabelas compartilhadas, como PESSOA, REGIMES, SOCIOS, etc, não podem ser
      recuperadas se o usuário utiliza mais de um programa }
    strLstTabelasCompartilhadas: TStringList;
    strLstTabelasCompartilhadasAmigavel: TStringList;

    { Caminho e nome do arquivo que apresenta as modificações do aplicativo. }
  ...
  TRodapeEmail = record
    licenca, nomePc, usuarioWindows, serialMother, ipLocal: String;
  end;
  ...
  { Neste registro contém os dados da empresa acessada pelo usuário. Ex: o usuário logou no
    programa contabil e entrou na empresa Modelo. No registro rSessaoInfo conterá as informações
    da empresa Modelo e tudo o mais relacionado à aquela sessão }
  {
    TInfoPessoa = record
    sNome : String; //ShortString
  ...
  // Registro de variáveis auxiliares
  TInfoAuxiliares = record
    iPCGrauMax: Integer; // Plano de contas - Grau Máximo
    sPCEstrutura: String; // ShortString //Plano de contas - Estrutura
    sPCMascara: String; // ShortString //Plano de contas - Máscara
    sContaSel: String;
    // ShortString  //Conta selecionada do plano de contas em FSelConta
    sDgContaSel: String;
    // ShortString //Dígito da conta selecionada em FSelConta
    iCdHistSel: Integer; // Código de histórico selecionado em FSelHist
    sIdContaSel: String; // ShortString //ID da conta selecionada
    sContaMasc: String; // ShortString //A conta contábil com a máscara
    ...
    { Tamanho máximo de uma arquivo de Logo para ser gravado no Firebird }
    iTamMaxArqFoto: Int64;
    { Tamanho máximo de uma arquivo de Foto para ser gravado no Firebird }

    // Nesta variavel será armazenada as informações sobre mes e ano para calculo ou lançamento.
...
procedure IniRodapeEmail(licenca, ip, nomePc, usuarioWindows,
  serialMae: String); { Inicializa as variaveis globais para rodape do email }
...
// Inicializa as variaveis com os dados da empresa para o Megacontabil que não tem SQLCONEXAO
...
procedure IniRodapeEmail(licenca, ip, nomePc, usuarioWindows,
  serialMae: String); { Inicializa as variaveis globais }
begin
  { Rodape email }
  {
...
begin
  ...
  try
    { marquei o SetDisableAutoDaylightTimeSet porque na máquina de D. Meire está dando erro }
    // SetDisableAutoDaylightTimeSet(false); //Desabilita o horário de verão
