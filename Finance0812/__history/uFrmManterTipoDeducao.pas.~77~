...
uses
  ...
  dxRibbonCustomizationForm, dxStatusBar, cxClasses, dxRibbon, dxBar,  uContaContabilModel, uHistoricoModel,
...
type
  TfrmManterTipoDeducao = class(TForm)
...
var
  frmManterTipoDeducao: TfrmManterTipoDeducao;

implementation

{$R *.dfm}

procedure TfrmManterTipoDeducao.cxbrdtmPesquisaCurChange(Sender: TObject);
begin
  dbgrd1.DataSource.DataSet.Locate('DESCRICAO',UpperCase(cxbrdtmPesquisa.CurEditValue),[loPartialKey]);
end;

procedure TfrmManterTipoDeducao.dbgrd1TitleClick(Column: TColumn);
begin
 (dbgrd1.DataSource.DataSet as TFDQuery).IndexFieldNames :=Column.FieldName;
end;

function TfrmManterTipoDeducao.deletar(pIdOrganizacao, pIdValue: string): Boolean;
var cmd :string;
qryDel :TFDQuery;
begin
try
...
procedure TfrmManterTipoDeducao.dsGridDataChange(Sender: TObject;
  Field: TField);
  var
...
begin
    pIdTipo                :=  (dbgrd1.DataSource.DataSet as TFDQuery).FieldByName('ID_TIPO_DEDUCAO').AsString;
...
procedure TfrmManterTipoDeducao.dxBtnDeletarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoDeducao.dxBtnEditarClick(Sender: TObject);
begin
...
procedure TfrmManterTipoDeducao.dxBtnFecharClick(Sender: TObject);
begin
 PostMessage(Self.Handle, WM_CLOSE, 0, 0);
end;

procedure TfrmManterTipoDeducao.dxBtnLimparClick(Sender: TObject);
begin
limparPanel;
end;

procedure TfrmManterTipoDeducao.dxBtnNovoClick(Sender: TObject);
begin
  limparPanel;
...
procedure TfrmManterTipoDeducao.dxBtnSalvarClick(Sender: TObject);
var
 conta :TContaContabilModel;
...
begin
  ...
  if not uUtil.Empty(historico.FIdContaContabil) then begin
    conta := TContaContabilModel.Create;
    conta := TContaContabilModel.Create;
    conta := frmContaContabil1.obterPorID(pIdOrganizacao, historico.FIdContaContabil);

    if not uutil.Empty(conta.FIdContaContabil) then
...
procedure TfrmManterTipoDeducao.edtTipoExit(Sender: TObject);
begin
...
procedure TfrmManterTipoDeducao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 freeAndNilDM(Self);
 Action :=caFree;
...
procedure TfrmManterTipoDeducao.FormCreate(Sender: TObject);
begin
 pTable :='TIPO_DEDUCAO';
   dxRibbon1Tab1.Caption := 'Manutenção de Deduções';
  pTipoHistorico := 'D'; //para preencher o frame historico
 inicializarDM(Self);
end;

procedure TfrmManterTipoDeducao.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 case key of
...
procedure TfrmManterTipoDeducao.frameHistorico1cbbcomboChange(Sender: TObject);
var
conta : TContaContabilModel;
historico :THistoricoModel;
begin
...
procedure TfrmManterTipoDeducao.freeAndNilDM(Sender: TObject);
begin

//nada
end;

procedure TfrmManterTipoDeducao.frmContaContabil1cbbContaContabilChange(
  Sender: TObject);
  var
  conta : TContaContabilModel;
begin
...
procedure TfrmManterTipoDeducao.inicializarDM(Sender: TObject);
begin
 //nada
...
procedure TfrmManterTipoDeducao.limparPanel();
begin
 limpaStatusBar;
...
procedure TfrmManterTipoDeducao.msgStatusBar(pPosicao : Integer; msg :string);
begin
...
procedure TfrmManterTipoDeducao.preencheGrid(pIdOrganizacao: string);
var
sqlCmd :string;
begin
...
dmConexao.conectarBanco;
 try
    ...
    sqlCmd := ' SELECT  D.DESCRICAO, D.ID_TIPO_DEDUCAO, '+
              ' HST.ID_HISTORICO, '+
              ' HST.ID_ORGANIZACAO, '+
...
function TfrmManterTipoDeducao.qtdUso(pIdOrganizacao, pIdvalue: string): Integer;
  var
...
begin
  ...
  try
    ...
    cmd := ' SELECT  FIRST 1 TP.ID_TITULO_PAGAR_BAIXA_DE  FROM TITULO_PAGAR_BAIXA_DE TP WHERE (TP.ID_ORGANIZACAO = :PIDORGANIZACAO) AND (TP.ID_'+ ptable + ' = :PID) ';
    ...
    cmd := ' SELECT  FIRST 1 TP.ID_TITULO_RECEBER_BAIXA_DE  FROM TITULO_RECEBER_BAIXA_DE TP WHERE (TP.ID_ORGANIZACAO = :PIDORGANIZACAO) AND (TP.ID_'+ ptable + ' = :PID) ';
...
procedure TfrmManterTipoDeducao.limpaStatusBar;
begin
dxStatusBar.Panels[0].Text := 'Status ';
dxStatusBar.Panels[1].Text := 'Ativo. Teclas de atalho:  [F2] = Editar  - [F4] = Novo - [F10] = Salvar  ';
end;

function TfrmManterTipoDeducao.obterIndiceHistorico(
...
function TfrmManterTipoDeducao.salvarTipo(pIdOrganizacao,pIdTipo, pIdHistorico, pDescricao: string): Boolean;
