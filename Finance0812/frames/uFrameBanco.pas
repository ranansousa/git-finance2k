...
type
  ...
  public
    ...
    function obterTodosPorIndice(pIdOrganizacao: string; var combo: TComboBox; var listaID: TStringList): boolean;
...
begin
  ...
  try
    cmd := ' SELECT  B.ID_BANCO, B.INDICE, B.CODIGO_BANCO, B.NOME_BANCO, B.SIGLA_BANCO, B.ID_ORGANIZACAO '+
...
begin
  ...
  try
    ...
    cmd := ' SELECT B.ID_BANCO, B.INDICE,  ' +
           ' B.ID_ORGANIZACAO, ' +
           ' B.CODIGO_BANCO,      ' +
           ' B.NOME_BANCO ,         ' +
           ' B.SIGLA_BANCO          ' +
           ' FROM BANCO B ' +
           ' WHERE  (B.ID_BANCO = :PID)';
...
function TfrmBanco.obterTodosPorIndice(pIdOrganizacao: string; var combo: TComboBox; var listaID: TStringList): boolean;
var
...
begin
   ...
   cmd := ' SELECT (B.NOME_BANCO) AS DSC, B.ID_BANCO '+
          ' FROM BANCO B WHERE (1=1) ORDER BY INDICE DESC, B.NOME_BANCO';
