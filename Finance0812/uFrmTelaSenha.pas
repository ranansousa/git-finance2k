...
procedure TfrmTelaSenha.btnFecharClick(Sender: TObject);
begin
...
inherited Create(AOwner);
 msgStatusBar(3,'Aguardando a senha...');
  ...
  begin
     ...
     if not uUtil.Empty(usuario.FsuperAdm) then begin
        if usuario.FsuperAdm.Equals('86b8e530-a539-11e3-aac2-c48c63baff9e') then begin
           autorizado := True;
           sucesso :=True;
     ...
     end else begin usuario.FsuperAdm := '1'; end;


    if not uutil.Empty(usuario.Fnome) then    begin
...
begin
  ...
  if verificaSenha(edtPassword.Text) then begin
    ...
    PempecMsg.MsgInformation('Funções liberadas.' + #13 +  ' Por favor, clique no botão ''OK'' e  aguarde o fechamento da tela...');
...
procedure TfrmTelaSenha.FormCreate(Sender: TObject);
begin
