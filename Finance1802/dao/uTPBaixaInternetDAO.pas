...
uses
  ...
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf, uBancoDAO, uBancoModel,
...
type
 TTPBaixaInternetDAO = class
  ...
  public
    ...
    class function deletePorTPBaixa(value :TTPBaixaInternetModel): Boolean;
...
class function TTPBaixaInternetDAO.deletePorTPBaixa(value: TTPBaixaInternetModel): Boolean;
var
qryDelete : TFDQuery;
sucesso :Boolean;
begin
  ...
  cmdSql :=   ' INSERT INTO TITULO_PAGAR_BAIXA_INTERNET '+
    ...
    begin
      qryInsert.ParamByName('PID_BANCO_DESTINO').Value := null;
         ...
         banco.FID := value.FIDbancoDestino;
         banco := TBancoDAO.obterPorID(banco);

         if not uutil.Empty(banco.FID) then begin

            TBancoDAO.incIndice(banco);
