...
uses
  ...
  uFrameTipoAcrescimo, uTipoAcrescimoModel,  uTipoAcrescimoDAO , uTPBaixaACModel,uFrmTituloPagarClone,uTituloPagarHistoricoDAO,uTituloPagarCentroCustoDAO,
  uTPBaixaDEModel , uTipoDeducaoModel,  uTipoDeducaoDAO, FireDAC.Stan.Async,
  FireDAC.DApt, uFrameTipoDeducao ;
  ...
  try
   ...
   if baixaModel.FtipoBaixa.Equals('PARCIAL') then begin
    //clonar titulo com o novo
      ...
      tituloClone.FvalorAntecipado  := totalFP;
...
begin
 ...
 if not uutil.Empty(tituloPagar.FIDLoteTPB)  then begin
   ...
   ShowMessage('O titulo está provisionado e foi exportado. O sistema não permite pagamento parcial.');
 end;
 ...
 if not uutil.Empty(tituloPagar.FIDLoteContabil)  then begin
   ...
   cbbTipoPagto.ItemIndex := 0;
   ShowMessage('O titulo foi exportado. O sistema não permite pagamento parcial.');
 end;
...
begin
  ...
  qtdDateio :=  pTitulo.listaHistorico.Count;
  if qtdDateio = 1  then begin
    qtdDateio :=  pTitulo.listaCustos.Count;
  end;
