...
begin
 ...
 try
   ...
   qryPesquisa.ParamByName('DATAINICIAL').AsDate := frmPeriodo1.getDataInicial;
   qryPesquisa.ParamByName('DATAFINAL').AsDate := frmPeriodo1.getDataFinal;
   qryPesquisa.Open();
...
begin
  ...
  begin
   ...
   // dmMegaContabil.ajustaValorLoteContabil(idEmpresaSistemaContabil, lote, ano, valorDebito, valorCredito);
    ...
    if (dmExportaFinance.gravarLoteContabil('FNCEXP@' + ano + '@' + IntToStr(lote) + '@' + IntToStr(linha), idOrganizacao, IntToStr(lote), FormatDateTime('DD/MM/YYYY', frmPeriodo1.getDataInicial), FormatDateTime('DD/MM/YYYY', frmPeriodo1.getDataFinal), lista, pTable ,  pTipo, linha, valorDebito, valorCredito)) then
    begin
...
chequesEmtidos :=0;
  ...
  if obterChequesEmitidos(idOrganizacao) then begin
     ...
     if chequesEmtidos = 1 then begin
      ShowMessage('Existem cheques emtitidos no período informado. A exportação de títulos quitados não será permitida até que os cheques sejam compensados.');
     end;
