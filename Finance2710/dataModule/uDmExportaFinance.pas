...
linha, lancamentoCR, listaCR,lancamentoDB,listaDB, I :Integer;
begin

     linha :=0;    listaCR :=0;  lancamentoCR :=0;  listaDB :=0;  lancamentoDB :=0;
                ...
                if obterTPBHistorico(pID_ORG,pID) then begin
                  //obtendo o rateio de historicos e suas contas contabeis
                   ...
                   while not qryTPBHistorico.Eof do begin
                       ...
                       CDREDUZDEB  := uUtil.convertStrToInt(qryTPBHistorico.FieldByName('CDREDUZDEB').AsString);
       ...
       try
              if not Uutil.Empty(pRegistroProvisao) then begin // se tem registro provisao indica que o TP foi provisinando
                //conta debito = ID_conta_contabil_credito do TP (Fornecedor)
                     ...
                     CDREDUZCRE  := uUtil.convertStrToInt(query.FieldByName('CDREDUZCRE').AsString);
                ...
                if obterTRBHistorico(pID_ORG,pID) then begin
                  //obtendo o rateio de historicos e suas contas contabeis
                   ...
                   while not qryTRBHistorico.Eof do begin
                       ...
                       COMPL       := qryTRBHistorico.FieldByName('COMPL').AsString;

                     CDREDUZCRE  := uUtil.convertStrToInt(qryTRBHistorico.FieldByName('CDREDUZCRE').AsString);
                 ...
                 if (obterTRBAC(pID_ORG,pIDBAIXA)) then begin
                  ...
                  while not qryTRBAcrescimos.Eof do
                      begin
                           ...
                           COMPL       := qryTRBAcrescimos.FieldByName('COMPL').AsString;
                           CDREDUZCRE  := uUtil.convertStrToInt(qryTRBAcrescimos.FieldByName('CDREDUZCRE').AsString);
                  ...
                  if (obterTRBDE(pID_ORG,pIDBAIXA)) then begin
                   //pegando as deducoes
                      while not qryTRBDeducao.Eof do begin
                          ...
                          COMPL       := qryTRBDeducao.FieldByName('COMPL').AsString;
                          CDHIST      := qryTRBDeducao.FieldByName('CDHIST').AsInteger;

                          CDREDUZDEB  := uUtil.convertStrToInt(qryTRBDeducao.FieldByName('CDREDUZDEB').AsString);
                  ...
                  //OBTER FORMA DE PAGAMENTO CAIXA
                   ...
                   //pegando as deducoes
                      while not qryTRBCaixa.Eof do begin
                          ...
                          COMPL       := qryTRBCaixa.FieldByName('COMPL').AsString;
                          CDHIST      := qryTRBCaixa.FieldByName('CDHIST').AsInteger;

                          CDREDUZDEB  := uUtil.convertStrToInt(qryTRBCaixa.FieldByName('CDREDUZDEB').AsString);
                 ...
                 try
                   ...
                   //pegando as deducoes
                      while not qryTRBCheque.Eof do begin
                          ...
                          COMPL       := qryTRBCheque.FieldByName('COMPL').AsString;
                          CDHIST      := qryTRBCheque.FieldByName('CDHIST').AsInteger;

                          CDREDUZDEB  := uUtil.convertStrToInt(qryTRBCheque.FieldByName('CDREDUZDEB').AsString);
                  ...
                  try
                    ...
                    if (obterTRBINTERNET(pID_ORG,pID)) then begin
                      ...
                      while not qryTRBInternet.Eof do begin
                          ...
                          COMPL       := qryTRBInternet.FieldByName('COMPL').AsString;
                          CDHIST      := qryTRBInternet.FieldByName('CDHIST').AsInteger;

                          CDREDUZDEB  := uUtil.convertStrToInt(qryTRBInternet.FieldByName('CDREDUZDEB').AsString);
  ...
  try
    ...
    //linha do debito
         ...
         if not qryTRDB.IsEmpty then begin
                          ...
                          CDREDUZCRE  := uUtil.convertStrToInt(qryTRDB.FieldByName('CDREDUZCRE').AsString);
