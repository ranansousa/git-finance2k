...
uses
  ...
  System.Variants, System.Classes,  Vcl.Graphics,uTituloReceberModel,uTituloReceberDAO,
...
type
 ...
 function obterTodosPorPeriodo(pIdValue: string; pDataInicio, pDataFinal :TDateTime; var combo: TComboBox;   var listaID: TStringList): boolean;
 function obterTitulo(titulo :TTituloReceberModel):TTituloReceberModel;
 function obterRateioContabil(titulo :TTituloReceberModel):TFDQuery;
  function obterRateioCustos(titulo :TTituloReceberModel):TFDQuery;
...
function TframeTR.obterRateioContabil(titulo: TTituloReceberModel): TFDQuery;
var
cmd :string;
...
begin
 ...
 cmd := ' SELECT TP.ID_TITULO_RECEBER_HISTORICO,' +
        ' TP.ID_ORGANIZACAO,' +
        ' TP.ID_HISTORICO,' +
        ' H.DESCRICAO AS DSC_HIST,'+
        ' TP.ID_TITULO_RECEBER,' +
        ' TP.VALOR' +
        ' FROM TITULO_RECEBER_HISTORICO TP' +
...
function TframeTR.obterRateioCustos(titulo: TTituloReceberModel): TFDQuery;
var
cmd :string;
qryPesquisa :TFDQuery;
lista :TStringList;
...
begin
 ...
 cmd := ' SELECT TPRC.ID_TITULO_RECEBER_RATEIO_CC,' +
        ' TPRC.ID_ORGANIZACAO,' +
        ' TPRC.ID_CENTRO_CUSTO,' +
        ' CC.DESCRICAO AS DSC_CC,'+
        ' TPRC.ID_TITULO_RECEBER,' +
        ' TPRC.VALOR' +
        ' FROM TITULO_RECEBER_RATEIO_CC TPRC' +
        ...
        '   AND (TPRC.ID_TITULO_RECEBER = :PID) ' +
...
function TframeTR.obterTitulo(titulo: TTituloReceberModel): TTituloReceberModel;
var
auxTitulo :TTituloReceberModel;
 ...
 try
  qryObterPorID := TFDQuery.Create(Self);
   ...
   if not qryObterPorID.IsEmpty then begin
      ...
      auxTitulo := TTituloReceberModel.Create;
      auxTitulo.FID                     := qryObterPorID.FieldByName('ID_TITULO_RECEBER').AsString;
      ...
      auxTitulo.FIDSacado              := qryObterPorID.FieldByName('ID_SACADO').AsString;
      ...
      auxTitulo.FIDUsuario              := qryObterPorID.FieldByName('ID_USUARIO').AsInteger;
      auxTitulo.FIDTituloReceberAnterior  := qryObterPorID.FieldByName('ID_TITULO_GERADOR').AsString;
      auxTitulo.FIDLoteContabil         := qryObterPorID.FieldByName('ID_LOTE_CONTABIL').AsString;
      auxTitulo.FIDLoteTRB              := qryObterPorID.FieldByName('ID_LOTE_TRB').AsString;
...
function TframeTR.obterTodos(pIdValue: string; var combo: TComboBox;  var listaID: TStringList): boolean;
var
cmdSql :string;
...
pCampoID := 'ID_'+ pTable;
 ...
 cmdSql := 'SELECT ' + pCampoSelecao +',' + pCampoID  +
           ...
           ' ORDER BY '+ pSigla + '.DATA_REGISTRO DESC' ;
...
function TframeTR.obterTodosPorPeriodo(pIdValue: string; pDataInicio,   pDataFinal: TDateTime; var combo: TComboBox; var listaID: TStringList): boolean;
var
cmdSql :string;
pSigla,pTable, pCampoSelecao, pCampoID :string;
begin
...
pCampoID := 'ID_'+ pTable;
 ...
 cmdSql := 'SELECT ' + pCampoSelecao +',' + pCampoID  +
           ' FROM ' + pTable + ' '+ pSigla +
           ' WHERE ( ' + pSigla + '.ID_ORGANIZACAO = :PIDORGANIZACAO) ' +
           ' AND   ( ' + pSigla + '.DATA_REGISTRO  BETWEEN :PDTINICIO AND :PDTFIM ) ' +
           ' AND   ( ' + pSigla + '.ID_TIPO_STATUS  <> ''EXCLUIDO'' ) ' +
           ' ORDER BY '+ pSigla + '.DATA_REGISTRO desc , TP.NUMERO_DOCUMENTO ' ;
 ...
 try
   ...
   qryObterTodos.SQL.Add(cmdSql);
   qryObterTodos.ParamByName('PIDORGANIZACAO').AsString := idOrganizacao;
   qryObterTodos.ParamByName('PDTINICIO').AsDate   := pDataInicio;
   qryObterTodos.ParamByName('PDTFIM').AsDate   := pDataFinal;
