...
begin
      ...
      frmCartaoCredito1.cbbBanco.ItemIndex := 0;
     ...
     cedenteModel := obterCedentePorId(pIdOrganizacao, pIdCedente);

     if uutil.Empty(cedenteModel.FID) then begin
       ShowMessage('Não foi possível obter os dados do cedente. Verifique com o suporte.');
       Self.Close;
     end;
 ...
 try
  //SALVADR ENDERECO
    ...
    // Salvar CONTATO
      ...
      cedente.Fnome                 := edtNomeCedente.Text;
    ...
    on e: Exception do
      ShowMessage(e.Message + sLineBreak + ' Não foi possível salvar o cedente.');
...
begin
 ...
 posicao :=0;
 frmCartaoCredito1.obterTodos(pIdOrganizacao, frmCartaoCredito1.cbbBanco, FsListaCartaoCredito);




 for I := 0 to FsListaCartaoCredito.Count-1 do begin
        if FsListaCartaoCredito[I].Equals(pIdCartaoCredito) then begin
          posicao := I;
          Break;
        end;
 end;

  frmCartaoCredito1.cbbBanco.ItemIndex := posicao;
